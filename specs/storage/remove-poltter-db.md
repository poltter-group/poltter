# Remove `packages/poltter/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/poltter/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the poltter legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/poltter/src/storage/db.ts` are concentrated in 22 source files:

- `packages/poltter/src/account/repo.ts`
- `packages/poltter/src/cli/cmd/db.ts`
- `packages/poltter/src/cli/cmd/import.ts`
- `packages/poltter/src/cli/cmd/stats.ts`
- `packages/poltter/src/control-plane/workspace.ts`
- `packages/poltter/src/index.ts`
- `packages/poltter/src/node.ts`
- `packages/poltter/src/permission/index.ts`
- `packages/poltter/src/project/project.ts`
- `packages/poltter/src/server/projectors.ts`
- `packages/poltter/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/poltter/src/server/shared/fence.ts`
- `packages/poltter/src/session/message-v2.ts`
- `packages/poltter/src/session/projectors.ts`
- `packages/poltter/src/session/prompt.ts`
- `packages/poltter/src/session/session.ts`
- `packages/poltter/src/session/todo.ts`
- `packages/poltter/src/share/share-next.ts`
- `packages/poltter/src/storage/db.ts`
- `packages/poltter/src/sync/index.ts`
- `packages/poltter/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy poltter database wrapper; `packages/poltter/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the poltter wrapper.
...
Status: Completed. `SyncEvent` and the poltter projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy poltter database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy poltter database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy poltter database wrapper.
...
All migration groups are complete or superseded. `packages/poltter/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No poltter data-migration group remains.

The previous poltter `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate poltter data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/poltter`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/poltter/src`
- `bun typecheck` from `packages/poltter`
- Relevant package tests from `packages/poltter`, not the repo root
