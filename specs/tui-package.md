...
Move the canonical Poltter terminal application from
`packages/poltter/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/poltter`,
`packages/cli`, or `@poltter-ai/core`.

The SDK is the TUI's Poltter boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/poltter`.
- Do not replace private `packages/poltter` imports with `@poltter-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@poltter-ai/tui`.
...
- Poltter domain data displayed by the TUI
...
```text
packages/poltter/src/cli/cmd/tui
```

Its private dependency on `packages/poltter` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/poltter/src/*`.
...
Status: Completed. The private `@poltter-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@poltter-ai/tui`.
...
- The package has no dependency on `poltter`, `@poltter-ai/cli`, or
  `@poltter-ai/core`.
...
primitives now live in `@poltter-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@poltter-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Poltter globals or process state;
...
Replace process-global Poltter state with resolved TUI inputs.
...
`@poltter-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/poltter` initially.
...
canonical in `@poltter-ai/tui`. Configured references resolve through the new
...
adapters rather than Poltter domain implementations.

Make the SDK the only Poltter domain boundary used by the TUI.
...
- Domain-facing TUI code imports Poltter data and operations only from
  `@poltter-ai/sdk`.
...
`@poltter-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/poltter`.
...
- The package has no imports from `packages/poltter`, `packages/cli`, or
  `@poltter-ai/core`.
...
public `@poltter-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/poltter`.
- Keep the legacy embedded worker and server startup in `packages/poltter`.
- Change those adapters to load config, create transport inputs, and call the
  public `@poltter-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/poltter`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@poltter-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@poltter-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Poltter domain state.
...
cd packages/poltter && bun typecheck
...
rg '@poltter-ai/core|packages/poltter|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/poltter/src packages/poltter/test
```
...
- Build the current-platform `packages/poltter` binary.
