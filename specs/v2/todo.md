...
The poltter server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@poltter-ai/llm` tool loop after replacing its
...
- plugins get global "poltter" instance like in that post i showed
- poltter instance has stuff like `poltter.session.prompt()` or
  `poltter.tool.register({...})`
...
- decide whether to preserve deprecated `@poltter-ai/llm` orchestration exports
