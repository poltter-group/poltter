...
Both `action` and `resource` use poltter's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.poltter` files is intentionally deferred until `.poltter` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether poltter permits an operation through its managed execution paths.
