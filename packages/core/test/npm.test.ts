...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@poltter/acme")).toBe("@poltter/acme")
    expect(Npm.sanitize("@poltter/acme@1.0.0")).toBe("@poltter/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/poltter/acme.git"
    const expected = win ? "acme@git+https_//github.com/poltter/acme.git" : spec
