...
import { define } from "@poltter-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@poltter-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "poltter.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "poltter.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
