...
import { FileSystemWatcher } from "@poltter-ai/schema/filesystem-watcher"
...
declare const POLTTER_LIBC: string | undefined
...
const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
  try {
    const libc = typeof POLTTER_LIBC === "undefined" ? undefined : POLTTER_LIBC
...
export class Service extends Context.Service<Service, Interface>()("@poltter/v2/FileWatcher") {}

const layer = Layer.effect(
  Service,
  Effect.gen(function* () {
    if (yield* Flag.POLTTER_EXPERIMENTAL_DISABLE_FILEWATCHER) return Service.of({})
    ...
    if (location.vcs && (yield* Flag.POLTTER_EXPERIMENTAL_FILEWATCHER)) {
