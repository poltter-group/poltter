...
import { ToolFailure } from "@poltter-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@poltter/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.POLTTER_WEBSEARCH_PROVIDER === "exa" || process.env.POLTTER_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.POLTTER_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("POLTTER_EXPERIMENTAL") || truthy("POLTTER_ENABLE_EXA") || truthy("POLTTER_EXPERIMENTAL_EXA"),
    enableParallel: truthy("POLTTER_ENABLE_PARALLEL") || truthy("POLTTER_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `poltter/${InstallationVersion}`,
