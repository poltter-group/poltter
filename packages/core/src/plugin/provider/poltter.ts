...
import type { IntegrationOAuthMethodRegistration } from "@poltter-ai/plugin/v2/effect/integration"
import { define } from "@poltter-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@poltter-ai/sdk/v2/types"
...
const defaultServer = "https://console.poltter.ai"
const clientID = "poltter-cli"
...
export const PoltterPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "poltter",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("PoltterPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("poltter")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Poltter provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("poltter", (integration) => {
        integration.name = "Poltter"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "poltter", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("poltter")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("poltter")
      ...
      const item = catalog.provider.get(ProviderV2.ID.poltter)
      if (!item) return
      const hasKey = Boolean(process.env.POLTTER_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("poltter")),
