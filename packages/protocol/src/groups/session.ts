...
import { PromptInput } from "@poltter-ai/schema/prompt-input"
import { Session } from "@poltter-ai/schema/session"
import { Project } from "@poltter-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@poltter-ai/schema/schema"
...
import { SessionEvent } from "@poltter-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Poltter process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Poltter process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
