import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@poltter-ai/sdk/v2"
import { SessionTurn } from "@poltter-ai/session-ui/session-turn"
import { SessionReview } from "@poltter-ai/session-ui/session-review"
import { DataProvider } from "@poltter-ai/session-ui/context"
import { FileComponentProvider } from "@poltter-ai/ui/context/file"
import { WorkerPoolProvider } from "@poltter-ai/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@poltter-ai/ui/logo"
import { IconButton } from "@poltter-ai/ui/icon-button"
import { ProviderIcon } from "@poltter-ai/ui/provider-icon"
import { iife } from "@poltter-ai/core/util/iife"
...
import { MessageNav } from "@poltter-ai/session-ui/message-nav"
import { FileSSR } from "@poltter-ai/session-ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@poltter-ai/session-ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/poltter-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | Poltter</Title>
              </Show>
              <Meta name="description" content="poltter - The AI coding agent built for the terminal." />
