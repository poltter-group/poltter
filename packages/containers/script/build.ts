...
const reg = process.env.REGISTRY ?? "ghcr.io/poltter-group"
...
const setup = async () => {
  ...
  if (list.includes("poltter")) {
    await $`docker buildx use poltter`
    return
  }
  await $`docker buildx create --name poltter --use`
}
