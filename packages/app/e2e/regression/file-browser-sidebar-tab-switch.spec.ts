...
const directory = "C:/Poltter/FileBrowserSidebar"
...
async function setup(page: Page) {
  await mockPoltterServer(page, {
    ...
    provider: {
      ...
      connected: ["poltter"],
      default: { providerID: "poltter", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
  ...
  await page.addInitScript(
    ({ directory, server, sessionID }) => {
      ...
      localStorage.setItem(
        "poltter.global.dat:layout",
        JSON.stringify({ review: { diffStyle: "split", panelOpened: true } }),
      )
      localStorage.setItem(
        "poltter.global.dat:review-panel-v2",
        JSON.stringify({ sidebarOpened: true, sidebarWidth: 240, expandMode: "collapse" }),
      )
      localStorage.setItem(
        "poltter.window.browser.dat:tabs",
