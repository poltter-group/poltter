...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-poltter" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslPoltterAction(check?: WslPoltterCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install Poltter"
  if (check.matchesDesktop === false) return "Update Poltter"
}
...
export function addServerViewModel(input: {
  ...
  const poltterCheck = selectedDistro ? (state?.poltterChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.poltterChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updatePoltter" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.poltterMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installPoltter" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  poltterCheck: WslPoltterCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingPoltter =
    input.state?.job?.kind === "install-poltter" && input.state.job.distro === input.selectedDistro
  if (!ready || probingPoltter) {
    return {
      variant: "contrast",
  ...
  if (!addServerPoltterReady(input.poltterCheck)) {
    const update = !!input.poltterCheck?.resolvedPath && input.poltterCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingPoltter
        ? { key: "wsl.onboarding.updatingPoltter" }
        : update
          ? { key: "wsl.onboarding.updatePoltter" }
          : { key: "wsl.onboarding.installPoltter" },
      disabled: !!input.state?.job || input.adding,
      action: "install-poltter",
      loading: installingPoltter,
...
function addServerPoltterReady(check: WslPoltterCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.poltterChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.poltterChecks[item.name]) return [`poltter:${item.name}`]
    return []
  })
