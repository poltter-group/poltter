...
import { Tag } from "@poltter-ai/ui/v2/badge-v2"
...
import { wslPoltterAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.poltterChecks[item.config.distro]
          const poltterAction = () => wslPoltterAction(check())
          const busy = () => wsl.data?.job?.kind === "install-poltter" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={poltterAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installPoltter(item.config.distro))}
                    >
