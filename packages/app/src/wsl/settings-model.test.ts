...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Poltter needs attention", () => {
    expect(wslPoltterAction(undefined)).toBeUndefined()
    expect(
      wslPoltterAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("Install Poltter")
    expect(
      wslPoltterAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/poltter",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("does not report ready when Poltter is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        poltterChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.poltter/bin/poltter",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installPoltter" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-poltter")
  })
