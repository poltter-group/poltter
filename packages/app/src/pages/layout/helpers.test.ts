...
import { type Session } from "@poltter-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("poltter://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("poltter://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("poltter://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("poltter://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("poltter://open-project")).toBeUndefined()
    expect(parseDeepLink("poltter://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "poltter://open-project?directory=/a",
      "poltter://other?directory=/b",
      "poltter://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("poltter://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("poltter://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("poltter://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("poltter://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "poltter://new-session?directory=/a",
      "poltter://open-project?directory=/b",
      "poltter://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __POLTTER__: {
        deepLinks: ["poltter://open-project?directory=/a"],
      },
    } as unknown as Window & { __POLTTER__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["poltter://open-project?directory=/a"])
