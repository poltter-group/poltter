...
import { NormalizedProviderListResponse } from "@poltter-ai/session-ui/context"
...
import type {
  ...
  McpServer,
  SessionActiveOutput,
} from "@poltter-ai/client/promise"
...
export const loadLspQuery = (scope: ServerScope, directory: string, sdk: PoltterClient) =>
  queryOptions({
...
function makeQueryOptionsApi(
  scope: ServerScope,
  serverSDK: () => PoltterClient,
  serverAPI: ServerApi,
  sdkFor: (dir: PathKey) => PoltterClient,
  protocol: Promise<"v1" | "v2">,
) {
  return {
...
export function createServerSyncContextInner(serverSDK: ServerSDK) {
  ...
  const sdkCache = new Map<string, PoltterClient>()
