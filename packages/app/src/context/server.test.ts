  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://poltter.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://poltter.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://poltter.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://poltter.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://poltter.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://poltter.example.com"),
      ),
    ).toEqual({
