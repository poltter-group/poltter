...
import type { retry } from "@poltter-ai/core/util/retry"
import type { PoltterEvent, SessionApi } from "@poltter-ai/client/promise"
import type { Message, PoltterClient, Part, Session } from "@poltter-ai/sdk/v2/client"
...
describe("server session", () => {
  test("projects V2 session events into current and legacy message state", () => {
    ...
    const apply = (input: object) => ctx.store.applyV2(input as PoltterEvent)
  ...
  test("extends a current page to include the user for split assistant turns", async () => {
    ...
    const store = createServerSession({} as PoltterClient, {} as SessionApi, messageApi)
    store.remember(session("root"))
