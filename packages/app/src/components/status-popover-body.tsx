import { Button } from "@poltter-ai/ui/button"
...
import { Tabs } from "@poltter-ai/ui/tabs"
...
import { DialogIntegrationApiKey, DialogIntegrationTelegram } from "./settings-v2/integration-dialog"
...
export function StatusPopoverBody(props: { shown: Accessor<boolean> }) {
  ...
  const pluginEmpty = createMemo(() => pluginEmptyMessage(language.t("dialog.plugins.empty"), "poltter.json"))

  const sdk = () => server.current ? global.ensureServerCtx(server.current).sdk : undefined

  const [tick, setTick] = createSignal(0)
  const [telegram] = createResource(
    () => [tick(), sdk()] as const,
    ([, s]) =>
      s?.api.integration.get({ integrationID: "telegram" })
  ...
  const [apiKey] = createResource(
    () => [tick(), sdk()] as const,
    ([, s]) =>
      s?.api.integration.get({ integrationID: "poltter" })
        .then((result) => result?.data),
  )
  ...
  const disconnectTelegram = async () => {
    ...
    try {
      await sdk()?.api.credential.remove({
  ...
  const integrations = [
    { id: "telegram", icon: "speech-bubble" as const, label: language.t("status.popover.integrations.telegram.title") },
    { id: "apikey", icon: "shield" as const, label: language.t("status.popover.integrations.apiKey.title") },
  ]
    ...
    <div class="flex items-center gap-1 w-[360px] rounded-xl shadow-[var(--shadow-lg-border-base)]">
      ...
      >
        <Tabs.List data-slot="tablist" class="bg-transparent border-b-0 px-4 pt-2 pb-0 gap-4 h-10">
          ...
          <Tabs.Trigger value="integrations" data-slot="tab" class="text-12-regular">
            {language.t("status.popover.tab.integrations")}
          </Tabs.Trigger>
        </Tabs.List>
              ...
              <Button
                ...
                onClick={() => {
                  void import("@/components/settings-v2").then((module) => {
                    dialog.show(() => <module.DialogSettings defaultValue="mcp" />)
                  })
                }}
              >
        ...
        <Tabs.Content value="integrations">
          <div class="flex flex-col px-2 pb-2">
            <div class="flex flex-col p-3 bg-background-base rounded-sm min-h-14">
              <div class="flex items-center gap-3 w-full min-h-10 px-3 py-2">
                <Icon name="speech-bubble" size="small" class="text-icon-base shrink-0" />
                <span class="text-14-regular text-text-base flex-1">{language.t("status.popover.integrations.telegram.title")}</span>
                <ButtonV2
                  size="small"
                  variant={telegramConnected() ? "danger" : "outline"}
                  onClick={() => (telegramConnected() ? void disconnectTelegram() : dialog.push(() => <DialogIntegrationTelegram />, () => setTick((value) => value + 1)))}
              ...
              <div class="flex items-center gap-3 w-full min-h-10 px-3 py-2">
                <Icon name="shield" size="small" class="text-icon-base shrink-0" />
                <span class="text-14-regular text-text-base flex-1">{language.t("status.popover.integrations.apiKey.title")}</span>
                <ButtonV2
                  size="small"
                  variant={apiKeyConnected() ? "danger" : "outline"}
                  onClick={() => (apiKeyConnected() ? void disconnectApiKey() : dialog.push(() => <DialogIntegrationApiKey />, () => setTick((value) => value + 1)))}
