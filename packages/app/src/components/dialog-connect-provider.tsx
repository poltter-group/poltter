import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@poltter-ai/client/promise"
...
import { List, type ListRef } from "@poltter-ai/ui/list"
import { ProviderIcon } from "@poltter-ai/ui/provider-icon"
import { Spinner } from "@poltter-ai/ui/spinner"
import { Tag } from "@poltter-ai/ui/tag"
import { TextField } from "@poltter-ai/ui/text-field"
import { ButtonV2 } from "@poltter-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@poltter-ai/ui/v2/dialog-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "poltter-go") return language.t("dialog.provider.poltterGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "poltter"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.poltter.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["poltter", "poltter-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "poltter" || provider.id === "poltter-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "poltter"
                                ? "dialog.provider.poltter.tagline"
                                : "dialog.provider.poltterGo.tagline",
                            )}
                          </span>
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "poltter"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.poltterZen.line1")}</div>
              <div>{language.t("provider.connect.poltterZen.line2")}</div>
              <div>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "poltter"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.poltterZen.line1")}</div>
