...
Создайте `.poltter/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию poltter или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем poltter [config](/docs/config):
...
- Глобальный: `~/.config/poltter/commands/`
- Для каждого проекта: `.poltter/commands/`
...
poltter включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
