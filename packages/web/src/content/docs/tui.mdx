---
title: TUI
description: Using the Poltter terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Poltter provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Poltter starts the TUI for the current directory.
...
When using the Poltter TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Poltter. Allows you to select from available providers and add their API keys.
...
This is separate from `poltter.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Poltter waits after the leader key. Defaults to `2000`.
...
Use `POLTTER_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `poltter.default`.
