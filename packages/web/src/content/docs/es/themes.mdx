...
Con Poltter puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Poltter usa nuestro propio tema `poltter`.
...
Poltter viene con varios temas integrados.
...
- Quiere que Poltter coincida con la apariencia de su terminal
...
Poltter admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/poltter/themes/*.json` o `$XDG_CONFIG_HOME/poltter/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.poltter/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.poltter/themes/*.json`
