...
Las habilidades del agente permiten a Poltter descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Poltter busca estas ubicaciones:

- Configuración del proyecto: `.poltter/skills/<name>/SKILL.md`
- Configuración global: `~/.config/poltter/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Poltter sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.poltter/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/poltter/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.poltter/skills/git-release/SKILL.md` así:
...
Poltter enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `poltter.json`:
...
**Para agentes integrados** (en `poltter.json`):
