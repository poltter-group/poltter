---
title: Servidor
description: Interactuar con el servidor poltter a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `poltter serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente poltter puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `poltter.local` |
...
Configure `POLTTER_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `poltter`, o configure `POLTTER_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `poltter serve` como a `poltter web`.
...
Cuando ejecuta `poltter`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor poltter para interactuar con poltter mediante programación.
:::

Esta arquitectura permite que poltter admita múltiples clientes y le permite interactuar con poltter mediante programación.

Puede ejecutar `poltter serve` para iniciar un servidor independiente. Si tienes el
poltter TUI ejecutándose, `poltter serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Poltter [IDE](/docs/ide).
...
El servidor poltter expone las siguientes API.
