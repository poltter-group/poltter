...
Las herramientas permiten que LLM realice acciones en su código base. Poltter viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Poltter.
...
:::note
Esta herramienta solo está disponible cuando `POLTTER_EXPERIMENTAL_LSP_TOOL=true` (o `POLTTER_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Poltter o cuando la variable de entorno `POLTTER_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Poltter:

```bash
POLTTER_ENABLE_EXA=1 poltter
```
