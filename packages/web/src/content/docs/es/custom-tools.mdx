...
description: Cree herramientas que LLM pueda llamar en poltter.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de poltter como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.poltter/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/poltter/tools/`.
...
```ts title=".poltter/tools/database.ts" {1}
...
```ts title=".poltter/tools/math.ts"
import { tool } from "@poltter-ai/plugin"
...
```ts title=".poltter/tools/bash.ts"
import { tool } from "@poltter-ai/plugin"
...
```ts title=".poltter/tools/project.ts" {8}
import { tool } from "@poltter-ai/plugin"
...
```python title=".poltter/tools/add.py"
import sys
...
```ts title=".poltter/tools/python-add.ts" {10}
import { tool } from "@poltter-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".poltter/tools/add.py")
