---
title: Complementos
description: Escriba sus propios complementos para extender Poltter.
---

Los complementos le permiten extender Poltter al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Poltter.
...
- `.poltter/plugins/` - Complementos a nivel de proyecto
- `~/.config/poltter/plugins/` - Complementos globales
...
```json title="poltter.json"
{
  ...
  "plugin": ["poltter-helicone-session", "poltter-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/poltter/node_modules/`.
...
1. Configuración global (`~/.config/poltter/poltter.json`)
2. Configuración del proyecto (`poltter.json`)
3. Directorio global de complementos (`~/.config/poltter/plugins/`)
4. Directorio de complementos del proyecto (`.poltter/plugins/`)
...
```json title=".poltter/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Poltter ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".poltter/plugins/my-plugin.ts"
...
```js title=".poltter/plugins/example.js"
...
- `client`: Un cliente SDK poltter para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar poltter.
...
```js title=".poltter/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "poltter"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Poltter, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que poltter lea archivos `.env`:

```javascript title=".poltter/plugins/env-protection.js"
...
```javascript title=".poltter/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a poltter:

```ts title=".poltter/plugins/custom-tools.ts"
import { type Plugin, tool } from "@poltter-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que poltter puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para poltter junto con las herramientas integradas.
...
```ts title=".poltter/plugins/compaction.ts"
import type { Plugin } from "@poltter-ai/plugin"
...
```ts title=".poltter/plugins/custom-compaction.ts"
import type { Plugin } from "@poltter-ai/plugin"
