...
Para depurar problemas con Poltter, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/poltter/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\poltter\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `poltter --log-level DEBUG`.

---

## Almacenamiento

poltter almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/poltter/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\poltter`
...
Poltter Desktop ejecuta un servidor Poltter local (el sidecar `poltter-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Poltter` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/poltter/poltter.jsonc` (o `~/.config/poltter/poltter.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/poltter/poltter.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\poltter\poltter.jsonc`
...
Poltter también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/poltter/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\poltter\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.poltter/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Poltter pueda reconstruirlo.

1. Salga de Poltter Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/poltter`
- **Linux**: eliminar `~/.cache/poltter` (o ejecutar `rm -rf ~/.cache/poltter`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\poltter`

3. Reinicie el escritorio Poltter.

---

### Solucionar problemas de conexión del servidor

Poltter Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `poltter.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `POLTTER_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `POLTTER_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Poltter requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Poltter.

---

### Notificaciones que no se muestran

Poltter Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Poltter en la configuración de su sistema operativo, y
...
1. Salga del escritorio Poltter.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Poltter):

- `poltter.settings.dat` (URL del servidor predeterminado de escritorio)
- `poltter.global.dat` y `poltter.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con Poltter:
...
3. Asegúrese de tener la última versión con `poltter upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `poltter models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/poltter
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\poltter`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. poltter instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\poltter`

2. Reinicie poltter para reinstalar los paquetes del proveedor más recientes.

Esto obligará a poltter a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
poltter detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
