...
Hay dos tipos de agentes en Poltter; agentes primarios y subagentes.
...
Poltter viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
Poltter viene con tres subagentes integrados, **General**, **Explore** y **Scout**. Veremos esto a continuación.

---

## Integrados

Poltter viene con dos agentes primarios integrados y tres subagentes integrados.
...
Un agente de solo lectura para investigar documentación externa y dependencias. Úsalo cuando necesites clonar el repositorio de una dependencia en la caché administrada de Poltter, inspeccionar el código fuente de una librería o contrastar el código local con implementaciones upstream sin modificar tu espacio de trabajo.
...
Configure agentes en su archivo de configuración `poltter.json`:
...
Si no se especifica ninguna temperatura, Poltter utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global Poltter como para la configuración específica del proyecto.
...
El ID del modelo en su configuración Poltter usa el formato `provider/model-id`. Por ejemplo, si usa [Poltter Zen](/docs/zen), usaría `poltter/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Ejecute `poltter models` para ver una lista de los modelos disponibles.
:::
