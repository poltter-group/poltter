---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć poltter.
---

Wtyczki umożliwiające wyłączenie poltter poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania poltter.
...
- `.poltter/plugins/` - Wtyczki na poziomie projektu
- `~/.config/poltter/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/poltter/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/poltter/poltter.json`)
2. Konfiguracja projektu (`poltter.json`)
3. Globalny katalog wtyczek (`~/.config/poltter/plugins/`)
4. Katalog wtyczek projektu (`.poltter/plugins/`)
...
poltter uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z poltter do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do poltter.
...
:::note
Jeśli korzystasz z aplikacji komputerowej poltter, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj poltter czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do poltter:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod poltter. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
