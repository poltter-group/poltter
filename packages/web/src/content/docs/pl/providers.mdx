---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w poltter.
---

import config from "../../../../config.mjs"
export const console = config.console

poltter używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji poltter.
...
w `~/.local/share/poltter/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim poltter
config.
...
Poltter Zen to lista modeli dostarczonych przez zespół poltter, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z poltter. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Poltter Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Poltter Zen` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
Działa jak każdy inny dostawca w poltter i jest całkowicie opcjonalny w użyciu.

---

## Poltter Go

Poltter Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół poltter, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z poltter.

1. Uruchom polecenie `/connect` w TUI, wybierz `Poltter Go` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
Aby używać Amazon Bedrock z poltter:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania poltter:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `poltter.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w poltter nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować poltter do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod poltter działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji poltter.
...
poltter używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować poltter
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`poltter.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem poltter:
...
Aby używać Google Vertex AI z poltter:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania poltter.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą poltter, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [poltter-helicone-session](https://github.com/H2Shami/poltter-helicone-session), aby automatycznie rejestrować każdą konwersację poltter jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja poltter będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować poltter tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować poltter tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować poltter tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem poltter. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/poltter).
:::
...
Aby korzystać z Ollama Cloud z poltter:
...
3. Skopiuj klucz API do użycia w poltter.
...
6. **Ważne**: Przed użyciem modeli chmurowych w poltter musisz pobrać informacje o modelu lokalnie:
...
Poltter Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół poltter. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Poltter Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Poltter Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API poltter.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji poltter.
...
5. Możesz je także dostosować za pomocą konfiguracji poltter. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z poltter:
...
Możesz także dostosować modele za pomocą konfiguracji poltter. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z poltter. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `poltter.json` w katalogu projektu:
...
Pola `limit` pozwalają poltter zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `poltter auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację poltter i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji poltter.
