...
Możesz dodać dodatkowe informacje do poltter za pomocą _Model Context Protocol_, w skrócie MCP. poltter obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [poltter Config](https://opencode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/poltter`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
poltter automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, poltter:
...
Jeśli serwer wymaga uwierzytelnienia, poltter poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `poltter mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji poltter bezpieczne przechowa tokeny w `~/.local/share/poltter/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w poltter, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie poltter, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć poltter z kontem Sentry.
