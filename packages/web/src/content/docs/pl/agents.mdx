...
W Poltter obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Poltter zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Poltter ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

Poltter ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej Poltter, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
Skonfiguruj agenta w pliku `poltter.json`:
...
- Globalnie: `~/.config/poltter/agents/`
- W projekcie: `.poltter/agents/`
...
Jeśli nie zostanie skonfigurowana, Poltter używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Poltter, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Poltter w `provider/model-id`. Na przykład, [Poltter Zen](/docs/zen), użyj `poltter/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `poltter models`, aby zobaczyć dostępne modele.
:::
