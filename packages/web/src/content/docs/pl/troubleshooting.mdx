...
Aby debugować problemy z poltter, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\poltter\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `poltter --log-level DEBUG`.

---

## Przechowywanie danych

poltter przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/poltter/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\poltter`
...
poltter Desktop uruchamia w tle lokalny serwer poltter (sidecar `poltter-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Poltter` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/poltter/poltter.jsonc` (lub `~/.config/poltter/poltter.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/poltter/poltter.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\poltter\poltter.jsonc`
...
poltter może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/poltter/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\poltter\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby poltter mógł ją odbudować.

1. Całkowicie zamknij poltter Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/poltter`
- **Linux**: usuń `~/.cache/poltter` (lub uruchom `rm -rf ~/.cache/poltter`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\poltter`

3. Uruchom ponownie poltter Desktop.

---

### Napraw problemy z połączeniem z serwerem

poltter Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `poltter.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `POLTTER_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `POLTTER_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows poltter Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami poltter.

---

### Brak powiadomień

poltter Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla poltter w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie poltter Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji poltter Desktop):

- `poltter.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `poltter.global.dat` i `poltter.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `poltter upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `poltter models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\poltter`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. poltter dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\poltter`

2. Uruchom ponownie kod poltter, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to poltter do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
poltter wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
