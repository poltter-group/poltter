...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Poltter posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Poltter.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `POLTTER_EXPERIMENTAL_LSP_TOOL=true` (lub `POLTTER_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Poltter lub gdy zmienna środowiskowa `POLTTER_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Poltter:
