---
title: Serwer
description: Interakcja z serwerem poltter za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `poltter serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient poltter.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `poltter.local` |
...
Ustaw `POLTTER_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `poltter` lub ustaw `POLTTER_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `poltter serve`, jak i `poltter web`.
...
Kiedy uruchomisz `poltter`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera poltter do programowej interakcji z poltter.
:::

Ta architektura umożliwia obsługę wielu klientów przez poltter i programową interakcję z kodem otwartym.

Możesz uruchomić `poltter serve`, aby uruchomić samodzielny serwer. Jeśli masz
poltter TUI działa, `poltter serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki poltter [IDE](/docs/ide).
...
Serwer poltter udostępnia następujące interfejsy API.
