...
Umiejętności agenta pozwalają poltter odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
poltter przeszukuje te lokalizacje:

- Project config: `.poltter/skills/<name>/SKILL.md`
- Global config: `~/.config/poltter/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu poltter przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.poltter/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/poltter/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.poltter/skills/git-release/SKILL.md` w ten sposób:
...
poltter wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `poltter.json`:
...
**Dla agentów wbudowanych** (w `poltter.json`):
