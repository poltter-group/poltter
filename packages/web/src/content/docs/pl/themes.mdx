...
Dzięki poltter możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie poltter używa naszego własnego motywu `poltter`.
...
poltter ma kilka wbudowanych motywów.
...
- Chcą, aby Poltter pasował do wyglądu ich terminala
...
poltter obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/poltter/themes/*.json` lub `$XDG_CONFIG_HOME/poltter/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.poltter/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.poltter/themes/*.json`
