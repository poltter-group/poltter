---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Poltter.
---

import config from "../../../../config.mjs"
export const console = config.console

Poltter utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Poltter.
...
en `~/.local/share/poltter/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Poltter
configuration.
...
Poltter Zen est une liste de modèles fournis par l'équipe Poltter qui ont été
testé et vérifié pour fonctionner correctement avec Poltter. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Poltter Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez poltter et dirigez-vous vers [opencode.ai/auth](https://opencode.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Poltter et son utilisation est totalement facultative.

---

## Poltter Go

Poltter Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Poltter qui ont été
testé et vérifié pour fonctionner correctement avec Poltter.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `Poltter Go` et rendez-vous sur [opencode.ai/auth](https://opencode.ai/zen).
...
Pour utiliser Amazon Bedrock avec Poltter :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de poltter :
...
Pour une configuration spécifique au projet ou persistante, utilisez `poltter.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Poltter n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer poltter pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que poltter fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration poltter.
...
pour stocker le jeton dans le stockage d'authentification poltter.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Poltter utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Poltter
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `poltter.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `poltter.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec poltter :
...
Pour utiliser Google Vertex AI avec Poltter :
...
Définissez-les lors de l'exécution de poltter.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via poltter, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [poltter-helicone-session](https://github.com/H2Shami/poltter-helicone-session) pour enregistrer automatiquement chaque conversation Poltter en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Poltter répertoriée comme une session distincte.
...
Vous pouvez configurer poltter pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer poltter pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer poltter pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Poltter. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/poltter) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec Poltter :
...
3. Copiez la clé API à utiliser dans Poltter.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Poltter, vous devez extraire les informations du modèle localement :
...
Poltter Zen est une liste de modèles testés et vérifiés fournie par l'équipe Poltter. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Poltter Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Poltter Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Poltter API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration poltter.
...
5. Vous pouvez également les personnaliser via votre configuration poltter. Voici un exemple de spécification d'un fournisseur
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Poltter :
...
Vous pouvez également personnaliser les modèles via votre configuration poltter. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec poltter. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `poltter.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à Poltter de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `poltter auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration poltter et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration poltter.
