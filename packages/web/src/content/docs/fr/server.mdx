---
title: Serveur
description: Interagissez avec le serveur poltter via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `poltter serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client poltter peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `poltter.local` |
...
Définissez `POLTTER_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `poltter`, ou définissez `POLTTER_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `poltter serve` et à `poltter web`.
...
Lorsque vous exécutez `poltter`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur poltter pour interagir avec poltter par programmation.
:::

Cette architecture permet à poltter de prendre en charge plusieurs clients et vous permet d'interagir avec poltter par programme.

Vous pouvez exécuter `poltter serve` pour démarrer un serveur autonome. Si vous avez le
poltter TUI en cours d'exécution, `poltter serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Poltter [IDE](/docs/ide).
...
Le serveur poltter expose les API suivantes.
