...
Avec Poltter, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Poltter utilise notre propre thème `poltter`.
...
Poltter est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Poltter corresponde à l'apparence de leur terminal
...
Poltter prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/poltter/themes/*.json` ou `$XDG_CONFIG_HOME/poltter/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.poltter/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.poltter/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/poltter/themes
vim ~/.config/poltter/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .poltter/themes
vim .poltter/themes/my-theme.json
```
