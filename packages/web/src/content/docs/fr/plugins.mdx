---
title: Plugins
description: Écrivez vos propres plugins pour étendre Poltter.
---

Les plugins vous permettent d'étendre Poltter en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Poltter.
...
- `.poltter/plugins/` - Plugins au niveau du projet
- `~/.config/poltter/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/poltter/node_modules/`.
...
1. Configuration globale (`~/.config/poltter/poltter.json`)
2. Configuration du projet (`poltter.json`)
3. Répertoire global des plugins (`~/.config/poltter/plugins/`)
4. Répertoire des plugins du projet (`.poltter/plugins/`)
...
Poltter exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK poltter pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre poltter.
...
:::note
Si vous utilisez l'application de bureau Poltter, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez poltter de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à poltter :
...
L'assistant `tool` crée un outil personnalisé que poltter peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour poltter aux côtés des outils intégrés.
