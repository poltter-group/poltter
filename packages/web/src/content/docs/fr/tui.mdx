---
title: TUI
description: Utilisation de l’interface utilisateur du terminal Poltter.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Poltter fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de Poltter démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le Poltter TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à Poltter. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Ceci est séparé de `poltter.json`, qui configure le comportement du serveur/d'exécution.
...
- `leader_timeout` - Contrôle la durée pendant laquelle Poltter attend après la leader key. Valeur par défaut : `2000`.
...
- `sound_pack` - ID du sound pack à utiliser. Valeur par défaut : `poltter.default`.
