...
Il existe deux types d'agents dans Poltter ; agents primaires et sous-agents.
...
Poltter est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Poltter est livré avec trois sous-agents intégrés, **General**, **Explore** et **Scout**. Nous les verrons ci-dessous.

---

## Agents intégrés

Poltter est livré avec deux agents primaires intégrés et trois sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `poltter.json` :
...
Si aucune température n'est spécifiée, Poltter utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Poltter et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Poltter utilise le format `provider/model-id`. Par exemple, si vous utilisez [Poltter Zen](/docs/zen), vous utiliserez `poltter/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `poltter models` pour voir une liste des modèles disponibles.
:::
