...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Poltter.
...
Voici plusieurs modèles qui fonctionnent bien avec Poltter, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Poltter.

```json title="poltter.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Poltter Zen](/docs/zen), vous utiliserez `poltter/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="poltter.jsonc" {7-12,19-24}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="poltter.jsonc" {6-21}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "poltter": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Poltter est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Poltter est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="poltter.jsonc" {7-18}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque Poltter démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Poltter.
