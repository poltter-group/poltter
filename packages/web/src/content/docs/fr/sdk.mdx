---
title: SDK
description: Client JS de type sécurisé pour le serveur poltter.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK poltter JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler poltter par programme.
...
Créez une instance de poltter :

```javascript
import { createPoltter } from "@poltter-ai/sdk"

const { client } = await createPoltter()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `poltter.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${poltter.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de poltter, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@poltter-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*poltter" },
})
