...
description: Lag verktøy som LLM kan kalle inn Poltter.
---

Egendefinerte verktøy er funksjoner du oppretter som LLM kan kalle under samtaler. De fungerer sammen med Poltter sine [innebygde verktøy](/docs/tools) som `read`, `write` og `bash`.
...
- Lokalt ved å plassere dem i `.poltter/tools/`-katalogen til prosjektet ditt.
- Eller globalt, ved å plassere dem i `~/.config/poltter/tools/`.
