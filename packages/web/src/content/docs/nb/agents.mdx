...
Det er to typer agenter i Poltter; primære agenter og underagenter.
...
Poltter kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Poltter kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Poltter kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `poltter.json`:
...
Hvis ingen temperatur er spesifisert, bruker Poltter modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Poltter-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Poltter-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Poltter Zen](/docs/zen), vil du bruke `poltter/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `poltter models` for å se en liste over tilgjengelige modeller.
:::
