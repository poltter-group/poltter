...
Agentferdigheter lar Poltter oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
poltter søker etter disse stedene:

- Prosjektkonfigurasjon: `.poltter/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/poltter/skills/<name>/SKILL.md`
...
For prosjektlokale stier går poltter opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.poltter/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/poltter/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.poltter/skills/git-release/SKILL.md` slik:
...
poltter viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `poltter.json`:
...
**For innebygde agenter** (i `poltter.json`):
