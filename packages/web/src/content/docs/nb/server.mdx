---
title: Tjener
description: Samhandle med Poltter-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `poltter serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Poltter-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `poltter.local` |
...
Sett `POLTTER_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `poltter` som standard, eller sett `POLTTER_SERVER_USERNAME` for å overstyre det. Dette gjelder både `poltter serve` og `poltter web`.
...
Når du kjører `poltter` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Poltter-serveren til å samhandle med Poltter programmatisk.
:::

Denne arkitekturen lar Poltter støtte flere klienter og lar deg samhandle med Poltter programmatisk.

Du kan kjøre `poltter serve` for å starte en frittstående server. Hvis du har
Poltter TUI kjører, vil `poltter serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Poltter [IDE](/docs/ide) plugins.
...
poltter-serveren viser følgende APIer.
