...
Du kan legge til eksterne verktøy i Poltter ved å bruke _Model Context Protocol_ eller MCP. Poltter støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Poltter Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="poltter.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/poltter`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Poltter håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Poltter:
...
Hvis serveren krever autentisering, vil Poltter be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `poltter mcp auth <server-name>`.
...
```bash
poltter mcp auth my-oauth-server
```
...
```bash
poltter mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Poltter lagre tokens sikkert i `~/.local/share/poltter/mcp-auth.json`.
...
poltter mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Poltter, sammen med innebygde verktøy. Så du kan administrere dem gjennom Poltter-konfigurasjonen som alle andre verktøy.
...
```json title="poltter.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
poltter mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Poltter til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Poltter-samtale oppført som en separat økt.
