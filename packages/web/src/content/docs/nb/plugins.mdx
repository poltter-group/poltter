...
description: Skriv dine egne programtillegg for å utvide Poltter.
---

Plugins lar deg utvide Poltter ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Poltter.
...
- `.poltter/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/poltter/node_modules/`.
...
1. Global konfigurasjon (`~/.config/poltter/poltter.json`)
2. Prosjektkonfigurasjon (`poltter.json`)
3. Global plugin-katalog (`~/.config/poltter/plugins/`)
4. Prosjektpluginkatalog (`.poltter/plugins/`)
...
Poltter kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Poltter SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Poltter.
...
:::note
Hvis du bruker Poltter-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Poltter fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Poltter:
...
`tool`-hjelperen lager et tilpasset verktøy som Poltter kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Poltter sammen med innebygde verktøy.
