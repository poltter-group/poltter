...
Create `.poltter/commands/test.md`:
...
You can add custom commands through the Poltter config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Poltter [config](/docs/config):
...
poltter includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
