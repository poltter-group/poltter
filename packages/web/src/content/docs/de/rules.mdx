---
title: Regeln
description: Lege eigene Anweisungen fuer poltter fest.
---

Du kannst poltter ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in poltter den Befehl `/init` aus.
...
So kann poltter sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht poltter sie zu erweitern.
...
poltter kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/poltter/AGENTS.md` speichern.
Diese gelten in allen poltter-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Poltter dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/poltter/AGENTS.md` existiert)
...
Beim Start sucht poltter in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/poltter/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `poltter.json` oder in `~/.config/poltter/poltter.json` angeben.
...
poltter parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit poltter.json

Empfohlen ist das `instructions`-Feld in der `poltter.json`:
...
Du kannst poltter explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `poltter.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
