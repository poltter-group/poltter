...
Wenn Poltter Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\poltter\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `poltter --log-level DEBUG`.

---

## Speicher

poltter speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/poltter/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\poltter`
...
Poltter Desktop startet im Hintergrund einen lokalen Poltter-Server (`poltter-cli`-Sidecar).
...
- Nur macOS: `Poltter`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/poltter/poltter.jsonc` (oder `~/.config/poltter/poltter.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/poltter/poltter.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\poltter\poltter.jsonc`
...
Poltter kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/poltter/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\poltter\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Poltter ihn neu aufbauen kann.

1. Beende Poltter Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/poltter`
- **Linux**: loesche `~/.cache/poltter` (oder fuehre aus: `rm -rf ~/.cache/poltter`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\poltter`

3. Starte Poltter Desktop neu.

---

### Server-Verbindungsprobleme beheben

Poltter Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `poltter.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `POLTTER_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `POLTTER_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Poltter Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Poltter-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Poltter Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Poltter in den OS-Einstellungen aktiviert sind, und
...
1. Beende Poltter Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Poltter Desktop):

- `poltter.settings.dat` (desktop default server URL)
- `poltter.global.dat` und `poltter.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Poltter hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `poltter models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\poltter`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. poltter installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\poltter`

2. Starte poltter neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt poltter, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
poltter erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
