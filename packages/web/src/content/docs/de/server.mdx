---
title: Server
description: Nutze den poltter-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `poltter serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein poltter-Client nutzen kann.
...
Setze `POLTTER_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `poltter`, kann aber mit `POLTTER_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `poltter serve` und `poltter web`.
...
Wenn du `poltter` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den poltter-Server, um poltter programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `poltter serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `poltter serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Poltter-[IDE](/docs/ide)-Plugins.
...
Der poltter-Server stellt folgende APIs bereit.
