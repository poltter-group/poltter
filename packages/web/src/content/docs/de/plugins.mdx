---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Poltter zu erweitern.
---

Mit Plugins können Sie Poltter erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Poltter zu ändern.
...
- `.poltter/plugins/` – Plugins auf Projektebene
- `~/.config/poltter/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/poltter/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/poltter/poltter.json`)
2. Projektkonfiguration (`poltter.json`)
3. Globales Plugin-Verzeichnis (`~/.config/poltter/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.poltter/plugins/`)
...
Poltter führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Poltter-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Poltter erweitern können.
...
:::note
Wenn Sie die Poltter-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Poltter `.env`-Dateien liest:
...
Plugins können Poltter auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Poltter aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Poltter zur Verfügung.
