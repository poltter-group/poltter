...
Es gibt zwei Arten von Agenten in Poltter; Primäragenten und Subagenten.
...
Poltter verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `poltter.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Poltter modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Poltter-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Poltter-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Poltter Zen](/docs/zen) verwenden, würden Sie `poltter/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `poltter models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
