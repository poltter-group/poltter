...
Erstellen Sie `.poltter/commands/test.md`:

```md title=".poltter/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die Poltter-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem Poltter [config](/docs/config):

```json title="poltter.jsonc" {4-12}
{
...
- Global: `~/.config/poltter/commands/`
- Pro Projekt: `.poltter/commands/`

```markdown title="~/.config/poltter/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".poltter/commands/component.md"
---
...
```md title=".poltter/commands/create-file.md"
---
...
```md title=".poltter/commands/analyze-coverage.md"
---
...
```md title=".poltter/commands/review-changes.md"
---
...
```md title=".poltter/commands/review-component.md"
---
...
Poltter enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
