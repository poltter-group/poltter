...
Agent Skills erlauben Poltter, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Poltter sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Poltter vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.poltter/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/poltter/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.poltter/skills/git-release/SKILL.md` zum Beispiel so:
...
Poltter listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `poltter.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `poltter.json`):
