...
Mit Poltter kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Poltter das `poltter`-Theme.
...
Poltter liefert mehrere eingebaute Themes mit.
...
- Poltter optisch ans Terminal anpassen wollen
...
Poltter unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/poltter/themes/*.json` oder `$XDG_CONFIG_HOME/poltter/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.poltter/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.poltter/themes/*.json`
