...
You can add external tools to Poltter using the _Model Context Protocol_, or MCP. Poltter supports both local and remote servers.
...
You can define MCP servers in your [Poltter Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/poltter` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Poltter automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Poltter will:
...
If the server requires authentication, Poltter will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `poltter mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Poltter will store the tokens securely in `~/.local/share/poltter/mcp-auth.json`.
...
Your MCPs are available as tools in Poltter, alongside built-in tools. So you can manage them through the Poltter config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Poltter to your Sentry account.
