---
title: Server
description: Interact with poltter server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `poltter serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an poltter client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `poltter.local` |
...
Set `POLTTER_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `poltter`, or set `POLTTER_SERVER_USERNAME` to override it. This applies to both `poltter serve` and `poltter web`.
...
When you run `poltter` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the poltter server to interact with poltter programmatically.
:::

This architecture lets poltter support multiple clients and allows you to interact with poltter programmatically.

You can run `poltter serve` to start a standalone server. If you have the
poltter TUI running, `poltter serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Poltter [IDE](/docs/ide) plugins.
...
The poltter server exposes the following APIs.
