...
With Poltter you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Poltter uses our own `poltter` theme.
...
Poltter comes with several built-in themes.
...
- Want Poltter to match their terminal's appearance
...
Poltter supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/poltter/themes/*.json` or `$XDG_CONFIG_HOME/poltter/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.poltter/themes/*.json`
4. **Current working directory** - Defined in `./.poltter/themes/*.json`
