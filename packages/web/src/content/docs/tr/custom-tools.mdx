---
title: Özel Araçlar
description: LLM'un poltter'da çağırabileceği araçları birleştirebilir.
---

Özel araçlar, LLM'un konuşmalar sırasında arayabileceği, oluşturduğunuz işlevlerdir. poltter'un [built-in tools](/docs/tools) ile birlikte `read`, `write` ve `bash` gibi çalışırlar.
...
- Yerel olarak bunları projenizin `.poltter/tools/` dizinine yerleştirerek.
- Veya küresel olarak bunları `~/.config/poltter/tools/` içine yerleştirerek.
