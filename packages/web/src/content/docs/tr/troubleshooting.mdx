...
poltter ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\poltter\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `poltter --log-level DEBUG`.

---

## Depolama

poltter, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/poltter/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\poltter` yapistirin
...
poltter Desktop arka planda yerel bir poltter sunucusu (`poltter-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `poltter` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/poltter/poltter.jsonc` (veya `~/.config/poltter/poltter.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/poltter/poltter.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\poltter\poltter.jsonc` yapistirin
...
poltter diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/poltter/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\poltter\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), poltter'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. poltter Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/poltter`
- **Linux**: `~/.cache/poltter` dizinini silin (veya `rm -rf ~/.cache/poltter` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\poltter` yapistirin

3. poltter Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

poltter Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`poltter.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `POLTTER_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `POLTTER_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta poltter Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, poltter ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

poltter Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda poltter icin bildirimler etkinse
...
1. poltter Desktop'u kapatin
2. Su dosyalari bulun ve silin (poltter Desktop uygulama veri dizininde yer alirlar):

- `poltter.settings.dat` (desktop varsayilan sunucu URL'si)
- `poltter.global.dat` ve `poltter.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
poltter ile ilgili bir sorun yasiyorsaniz:
...
3. `poltter upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `poltter models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\poltter`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. poltter, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\poltter`

2. En guncel provider paketlerini yeniden kurmak icin poltter'u yeniden baslatin

Bu, poltter'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
poltter, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
