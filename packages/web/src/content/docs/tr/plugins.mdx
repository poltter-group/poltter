---
title: Eklentiler
description: Poltter'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Poltter'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Poltter'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.poltter/plugins/` - Proje düzeyinde eklentiler
- `~/.config/poltter/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/poltter/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/poltter/poltter.json`)
2. Project config (`poltter.json`)
3. Global eklenti dizini (`~/.config/poltter/plugins/`)
4. Proje eklenti dizini (`.poltter/plugins/`)
...
Poltter bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Poltter SDK istemcisi.
...
Poltter'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Poltter masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Poltter'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Poltter'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Poltter'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Poltter'a da hazır olacaktır.
