...
Ajan becerileri, poltter'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
poltter su konumlari tarar:

- Proje konfigurasyonu: `.poltter/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/poltter/skills/<name>/SKILL.md`
...
Proje yerel yollarinda poltter, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.poltter/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/poltter/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.poltter/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
poltter, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`poltter.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`poltter.json` icinde):
