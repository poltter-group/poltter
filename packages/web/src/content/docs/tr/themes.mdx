...
poltter ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak poltter kendi `poltter` temasını kullanır.
...
poltter birden fazla yerleşik temayla gelir.
...
- poltter'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
poltter, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/poltter/themes/*.json` veya `$XDG_CONFIG_HOME/poltter/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.poltter/themes/*.json`
4. **Geçerli çalışma dizini** - `./.poltter/themes/*.json`
