...
Poltter Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Poltter'daki diğer sağlayıcılar gibi çalışır. Poltter Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Poltter'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Poltter Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Poltter Go, Poltter'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Poltter Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Poltter Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Poltter Go'ya abone olabilir.
:::
...
Poltter Go aşağıdaki limitleri içerir:
...
Poltter yapılandırmanızdaki [model id](/docs/config/#models) formatı `poltter-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `poltter-go/kimi-k3` kullanmalısınız.
...
Poltter Go'yu şu amaçlarla oluşturduk:
...
4. Poltter ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
