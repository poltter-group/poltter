---
title: Sağlayıcılar
description: poltter'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

poltter, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı poltter yapılandırmanızda yapılandırın.
...
`~/.local/share/poltter/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları poltter'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Poltter Zen, poltter ekibi tarafından sağlanan modellerin bir listesidir.
poltter ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Poltter Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, poltter'u seçin ve [opencode.ai/auth](https://opencode.ai/auth)'ye gidin.
...
poltter'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Poltter Go

Poltter Go, poltter ile iyi çalıştığı test edilmiş ve doğrulanmış, poltter ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Poltter Go`'yu seçin ve [opencode.ai/auth](https://opencode.ai/zen) adresine gidin.
...
Amazon Bedrock'u poltter ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   poltter'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `poltter.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi poltter'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
poltter'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   poltter'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Poltter yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
poltter, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. poltter'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`poltter.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`poltter.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi poltter'la kullanmak için:
...
Google Vertex AI'yi poltter ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları poltter çalıştırılırken ayarlayın.
...
Helicone'un poltter aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her poltter görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [poltter-helicone-session](https://github.com/H2Shami/poltter-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her poltter görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
poltter'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla poltter'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla poltter'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini poltter için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/poltter)'a bakın.
:::
...
Ollama Cloud'u poltter ile kullanmak için:
...
3. poltter'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: poltter'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Poltter Zen, poltter ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Poltter Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Poltter Zen**'i arayın.

   ```txt
   /connect
   ```

3. poltter API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   poltter yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca poltter yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Poltter ile kullanmak için:
...
Modelleri poltter yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı poltter ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `poltter.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları poltter'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `poltter auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için poltter yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, poltter yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
