...
_Model Bağlam Protokolü_ veya MCP kullanarak poltter'a harici araçlar ekleyebilirsiniz. poltter hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [poltter Config](https://opencode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/poltter` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
poltter, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde poltter şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa poltter, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `poltter mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından poltter, belirteçleri `~/.local/share/poltter/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra poltter'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi poltter yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve poltter'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
