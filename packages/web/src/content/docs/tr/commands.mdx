...
`.poltter/commands/test.md` oluştur:
...
poltter işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

poltter [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/poltter/commands/`
- Proje başına: `.poltter/commands/`
...
poltter, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
