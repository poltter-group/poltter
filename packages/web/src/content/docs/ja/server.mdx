---
title: サーバー
description: HTTP 経由で Poltter サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`poltter serve` コマンドは、Poltter クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `poltter.local` |
...
HTTP 基本認証でサーバーを保護するには、`POLTTER_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `poltter` になるか、`POLTTER_SERVER_USERNAME` を設定してオーバーライドします。これは、`poltter serve` と `poltter web` の両方に当てはまります。
...
`poltter` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Poltter サーバーを使用して、プログラムで Poltter と対話します。
:::

`poltter serve` を実行してスタンドアロンサーバーを起動できます。すでに Poltter TUI を実行している場合でも、`poltter serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Poltter [IDE](/docs/ide) プラグインによって使用されます。
