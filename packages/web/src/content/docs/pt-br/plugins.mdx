---
title: Plugins
description: Escreva seus próprios plugins para estender o poltter.
---

Plugins permitem que você estenda o poltter conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do poltter.
...
- `.poltter/plugins/` - Plugins em nível de projeto
- `~/.config/poltter/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/poltter/node_modules/`.
...
1. Configuração global (`~/.config/poltter/poltter.json`)
2. Configuração do projeto (`poltter.json`)
3. Diretório de plugins global (`~/.config/poltter/plugins/`)
4. Diretório de plugins do projeto (`.poltter/plugins/`)
...
O poltter executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do poltter para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o poltter.
...
:::note
Se você estiver usando o aplicativo desktop poltter, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o poltter de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao poltter:
...
O helper `tool` cria uma ferramenta personalizada que o poltter pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o poltter junto com as ferramentas integradas.
