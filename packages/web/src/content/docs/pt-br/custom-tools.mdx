...
description: Crie ferramentas que o LLM pode chamar no poltter.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do poltter, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.poltter/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/poltter/tools/`.
