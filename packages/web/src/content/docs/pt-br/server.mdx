---
title: Servidor
description: Interaja com o servidor poltter via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `poltter serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente poltter pode usar.
...
```bash
poltter serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `poltter.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
poltter serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `POLTTER_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `poltter`, ou defina `POLTTER_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `poltter serve` quanto ao `poltter web`.

```bash
POLTTER_SERVER_PASSWORD=your-password poltter serve
```
...
Quando você executa `poltter`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor poltter para interagir com o poltter programaticamente.
:::

Essa arquitetura permite que o poltter suporte múltiplos clientes e permite que você interaja com o poltter programaticamente.

Você pode executar `poltter serve` para iniciar um servidor autônomo. Se você tiver o TUI do poltter em execução, `poltter serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do poltter [IDE](/docs/ide).
...
O servidor poltter expõe as seguintes APIs.
