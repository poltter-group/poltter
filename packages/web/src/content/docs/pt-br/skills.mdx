...
As habilidades do agente permitem que o poltter descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O poltter pesquisa nesses locais:

- Configuração do projeto: `.poltter/skills/<name>/SKILL.md`
- Configuração global: `~/.config/poltter/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o poltter sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.poltter/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/poltter/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.poltter/skills/git-release/SKILL.md` assim:
...
O poltter lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `poltter.json`:
...
**Para agentes embutidos** (em `poltter.json`):
