...
Com o poltter, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o poltter usa nosso próprio tema `poltter`.
...
O poltter vem com vários temas embutidos.
...
- Querem que o poltter corresponda à aparência do seu terminal
...
O poltter suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/poltter/themes/*.json` ou `$XDG_CONFIG_HOME/poltter/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.poltter/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.poltter/themes/*.json`
