...
مع Poltter يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Poltter سمتنا `poltter`.
...
يأتي Poltter مع عدة سمات مدمجة.
...
- يريدون أن يطابق Poltter مظهر terminal لديهم
...
يدعم Poltter نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/poltter/themes/*.json` أو `$XDG_CONFIG_HOME/poltter/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.poltter/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.poltter/themes/*.json`
