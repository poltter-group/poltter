...
أنشئ الملف `.poltter/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Poltter أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Poltter:
...
- على مستوى النظام: `~/.config/poltter/commands/`
- لكل مشروع: `.poltter/commands/`
...
يتضمن poltter عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
