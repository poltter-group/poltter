...
تتيح مهارات الوكيل لـ Poltter اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Poltter في هذه المواقع:

- إعدادات المشروع: `.poltter/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/poltter/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Poltter صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.poltter/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/poltter/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.poltter/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Poltter المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `poltter.json`:
...
**للوكلاء المدمجين** (في `poltter.json`):
