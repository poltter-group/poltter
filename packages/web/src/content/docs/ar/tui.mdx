---
title: TUI
description: استخدام واجهة المستخدم TUI في Poltter.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يوفّر Poltter واجهة terminal تفاعلية (TUI) للعمل على مشاريعك باستخدام LLM.

يؤدي تشغيل Poltter إلى بدء واجهة TUI للدليل الحالي.
...
عند استخدام واجهة Poltter في terminal، يمكنك كتابة `/` متبوعة باسم أمر لتنفيذ الإجراءات بسرعة. مثلا:
...
أضف موفّرا إلى Poltter. يتيح لك اختيار أحد الموفّرين المتاحين وإضافة مفاتيح API الخاصة بهم.
...
اخرج من Poltter. _الأسماء المستعارة_: `/quit`, `/q`
...
هذا منفصل عن `poltter.json`، الذي يضبط سلوك الخادم ووقت التشغيل.
...
- `leader_timeout` - يتحكم في مدة انتظار Poltter بعد الضغط على leader key. القيمة الافتراضية `2000`.
...
- `sound_pack` - sound pack ID المراد استخدامه. القيمة الافتراضية `poltter.default`.
