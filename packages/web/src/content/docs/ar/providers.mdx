---
title: المزوّدون
description: استخدام أي موفّر LLM في Poltter.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Poltter ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Poltter.
...
في `~/.local/share/poltter/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Poltter.
...
Poltter Zen هي قائمة نماذج يوفّرها فريق Poltter وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Poltter. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Poltter Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر poltter، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/auth).
...
يعمل مثل أي مزوّد آخر في Poltter واستخدامه اختياري بالكامل.

---

## Poltter Go

Poltter Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Poltter، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Poltter.

1. شغّل الأمر `/connect` في TUI، واختر `Poltter Go`، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/zen).
...
لاستخدام Amazon Bedrock مع Poltter:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل poltter:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `poltter.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Poltter غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين poltter لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل poltter بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات poltter.
...
تخزين الرمز في مساحة تخزين مصادقة poltter.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Poltter نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Poltter
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`poltter.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع poltter:
...
لاستخدام Google Vertex AI مع Poltter:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر poltter، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [poltter-helicone-session](https://github.com/H2Shami/poltter-helicone-session) لتسجيل كل محادثة Poltter تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Poltter مدرجة كجلسة مستقلة.
...
يمكنك ضبط poltter لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط poltter لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط poltter لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Poltter. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/poltter) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Poltter:
...
3. انسخ مفتاح API لاستخدامه في Poltter.
...
6. **مهم**: قبل استخدام نماذج السحابة في Poltter، يجب سحب معلومات النموذج محليا:
...
Poltter Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Poltter. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Poltter Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Poltter Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Poltter.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات poltter.
...
5. يمكنك أيضا تخصيصها عبر إعدادات poltter. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Poltter:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات poltter. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع poltter. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `poltter.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Poltter بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `poltter auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات poltter وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات poltter.
