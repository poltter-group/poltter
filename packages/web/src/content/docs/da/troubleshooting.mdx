...
For at fejlfinde problemer med Poltter, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\poltter\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `poltter --log-level DEBUG`.

---

## Lagring

poltter gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/poltter/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\poltter`
...
Poltter Desktop kører en lokal Poltter-server (`poltter-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Poltter`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/poltter/poltter.jsonc` (eller `~/.config/poltter/poltter.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/poltter/poltter.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\poltter\poltter.jsonc`
...
Poltter kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/poltter/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\poltter\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Poltter kan genopbygge den.

1. Afslut Poltter Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/poltter`
- **Linux**: slet `~/.cache/poltter` (eller kør `rm -rf ~/.cache/poltter`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\poltter`

3. Start Poltter Desktop på ny.

---

### Løs problemer med serverforbindelse

Poltter Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `poltter.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `POLTTER_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `POLTTER_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Poltter Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Poltter.

---

### Meddelelser vises ikke

Poltter Desktop viser kun systemvarsler når:

- varsler er aktiveret for Poltter i dine OS-indstillinger, og
...
1. Afslut Poltter Desktop.
2. Find og slet disse filer (de findes i Poltter Desktop-appens datamappe):

- `poltter.settings.dat` (skrivebordsstandardserver URL)
- `poltter.global.dat` og `poltter.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Poltter:
...
For at finde ud af hvilke modeller du har adgang til, kør `poltter models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\poltter`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. poltter installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\poltter`

2. Start poltter på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge poltter til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
poltter vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
