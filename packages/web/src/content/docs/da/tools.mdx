...
Her er alle de innebygde verktøyene som er tilgængelige i Poltter.
...
:::note
Dette verktøyet er bare tilgængelig når `POLTTER_EXPERIMENTAL_LSP_TOOL=true` (eller `POLTTER_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Poltter-leverandøren eller når miljøvariabelen `POLTTER_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
