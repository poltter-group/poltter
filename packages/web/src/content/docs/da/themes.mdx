...
Med Poltter kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Poltter vårt eget `poltter`-tema.
...
Poltter støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/poltter/themes/*.json` eller `$XDG_CONFIG_HOME/poltter/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.poltter/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.poltter/themes/*.json`
