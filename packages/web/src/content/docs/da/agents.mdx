...
Der er to typer agenter i Poltter; primære agenter og subagenter.
...
Poltter leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
Poltter leveres med tre indbyggede subagenter, **General**, **Explore** og **Scout**. Vi ser nærmere på dem nedenfor.

---

## Indbyggede

Poltter leveres med to indbyggede primære agenter og tre indbyggede subagenter.
...
En skrivebeskyttet agent til eksterne docs og research af dependencies. Brug denne, når du har brug for at klone et dependency-repository ind i Poltter's administrerede cache, inspicere kildekoden i et bibliotek eller krydstjekke lokal kode mod upstream-implementeringer uden at ændre dit workspace.
...
Konfigurer agent i din `poltter.json`-konfigurationsfil:
...
- Globalt: `~/.config/poltter/agents/`
- Pr. projekt: `.poltter/agents/`
...
Hvis ingen temperatur er angivet, bruger Poltter modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale Poltter-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din Poltter-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [Poltter Zen](/docs/zen), vil du bruge `poltter/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `poltter models` for at se en liste over de tilgængelige modeller.
:::
