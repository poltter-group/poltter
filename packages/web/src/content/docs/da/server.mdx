---
title: Server
description: Interager med poltter-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `poltter serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en poltter-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `poltter.local` |
...
Indstil `POLTTER_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `poltter`, eller indstil `POLTTER_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `poltter serve` og `poltter web`.
...
Når du kører `poltter` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug poltter-serveren til at interagere med poltter programmatisk.
:::

Denne arkitektur lader poltter understøtte flere klienter og lader dig interagere med poltter programmatisk.

Du kan køre `poltter serve` for at starte en selvstændig server. Hvis du har
poltter TUI kørende, vil `poltter serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Poltter [IDE](/docs/ide) plugins.
...
Poltter-serveren viser følgende API'er.
