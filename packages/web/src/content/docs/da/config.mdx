---
title: Konfiguration
description: Ved at bruge Poltter JSON konfig.
---

Du kan konfigurere Poltter ved hjælp af en JSON-konfigurationsfil.

---

## Format

Poltter understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/poltter`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/poltter/poltter.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`POLTTER_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`poltter.json` i projekt) - projektspecifikke indstillinger
5. **`.poltter` mapper** - agent, kommandoer, plugins
6. **Inline config** (`POLTTER_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.poltter` og `~/.config/poltter` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/poltter`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale Poltter-konfiguration i `~/.config/poltter/poltter.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `poltter.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når Poltter starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `POLTTER_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `POLTTER_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.poltter` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.poltter` mapper, så den **kan tilsidesætte** deres indstillinger.
...
Du kan konfigurere serverindstillinger for kommandoerne `poltter serve` og `poltter web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din Poltter-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `poltter.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din Poltter-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger Poltter at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din Poltter-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/poltter/agents/` eller `.poltter/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil Poltter falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`poltter run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/poltter/commands/` eller `.poltter/commands/`. [Learn more here](/docs/commands).
...
Poltter vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard poltter **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide Poltter med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.poltter/plugins/` eller `~/.config/poltter/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse Poltter til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
