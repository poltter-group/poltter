---
title: Plugins
description: Skriv dine egne plugins for at udvide Poltter.
---

Plugins giver dig mulighed for at udvide Poltter ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Poltters standardadfærd.
...
- `.poltter/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/poltter/node_modules/`.
...
1. Global konfiguration (`~/.config/poltter/poltter.json`)
2. Projektkonfiguration (`poltter.json`)
3. Global plugin-mappe (`~/.config/poltter/plugins/`)
4. Projekt plugin bibliotek (`.poltter/plugins/`)
...
Poltter kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En poltter SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide poltter.
...
:::note
Hvis du bruger Poltter desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå poltter i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til poltter:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som poltter kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for poltter sammen med indbyggede værktøjer.
