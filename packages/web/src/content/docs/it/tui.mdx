---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Poltter.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Poltter fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Poltter avvia la TUI per la directory corrente.
...
Quando usi la TUI di Poltter, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Poltter. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Poltter. _Alias_: `/quit`, `/q`
...
Questo è separato da `poltter.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Poltter attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `poltter.default`.
