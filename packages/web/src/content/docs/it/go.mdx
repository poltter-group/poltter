...
Poltter Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in Poltter. Ti abboni a Poltter Go e
...
Poltter Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

Poltter Go funziona come qualsiasi altro provider in Poltter.

1. Accedi a **<a href={console}>Poltter Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Poltter Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Poltter Go.
:::
...
Poltter Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Poltter config
utilizza il formato `poltter-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`poltter-go/kimi-k3` nella tua configurazione.
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Poltter.
