...
Puoi aggiungere strumenti esterni a Poltter usando il _Model Context Protocol_, o MCP. Poltter supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Poltter Config](https://opencode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/poltter`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Poltter gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Poltter:
...
Se il server richiede autenticazione, Poltter ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `poltter mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Poltter memorizzera' i token in modo sicuro in `~/.local/share/poltter/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Poltter insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Poltter come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Poltter al tuo account Sentry.
