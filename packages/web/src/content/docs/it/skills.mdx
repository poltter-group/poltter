...
Le skill degli agenti permettono a Poltter di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Poltter cerca in queste posizioni:

- Config di progetto: `.poltter/skills/<name>/SKILL.md`
- Config globale: `~/.config/poltter/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Poltter risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.poltter/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/poltter/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.poltter/skills/git-release/SKILL.md` cosi':
...
Poltter elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `poltter.json`:
...
**Per agenti integrati** (in `poltter.json`):
