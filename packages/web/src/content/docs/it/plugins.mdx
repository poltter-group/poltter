---
title: Plugin
description: Scrivi plugin per estendere Poltter.
---

I plugin ti permettono di estendere Poltter agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Poltter.
...
- `.poltter/plugins/` - plugin a livello progetto
- `~/.config/poltter/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/poltter/node_modules/`.
...
1. Config globale (`~/.config/poltter/poltter.json`)
2. Config di progetto (`poltter.json`)
3. Directory plugin globale (`~/.config/poltter/plugins/`)
4. Directory plugin di progetto (`.poltter/plugins/`)
...
Poltter esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di poltter per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere poltter.
...
:::note
Se usi l'app desktop di Poltter, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a poltter di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a poltter:
...
L'helper `tool` crea uno strumento personalizzato che poltter può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in poltter insieme agli strumenti integrati.
