---
title: Server
description: Interagisci con il server poltter via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `poltter serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client poltter.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `poltter.local` |
...
Imposta `POLTTER_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `poltter`, oppure imposta `POLTTER_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `poltter serve` sia per `poltter web`.
...
Quando esegui `poltter` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server poltter per interagire con poltter in modo programmatico.
:::

Questa architettura permette a poltter di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `poltter serve` per avviare un server standalone. Se la TUI di poltter e gia in esecuzione, `poltter serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Poltter.
...
Il server poltter espone le seguenti API.
