...
Poltter Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

Go works like any other provider in Poltter. You subscribe to Poltter Go and
...
Poltter Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

Poltter Go works like any other provider in Poltter.

1. You sign in to **<a href={console}>Poltter Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `Poltter Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to Poltter Go.
:::
...
Poltter Go includes the following limits:
...
uses the format `poltter-go/<model-id>`. For example, for Kimi K3, you would
use `poltter-go/kimi-k3` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with Poltter as well.
