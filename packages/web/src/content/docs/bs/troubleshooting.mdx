...
Da biste otklonili probleme s Poltter, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\poltter\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `poltter --log-level DEBUG`.

---

## Pohrana

Poltter pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/poltter/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\poltter`
...
Poltter Desktop pokreće lokalni Poltter server (`poltter-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Poltter` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/poltter/poltter.jsonc` (ili `~/.config/poltter/poltter.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/poltter/poltter.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\poltter\poltter.jsonc`
...
Poltter također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/poltter/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\poltter\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Poltter mogao ponovo izgraditi.

1. Potpuno zatvorite Poltter Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/poltter`
- **Linux**: obrišite `~/.cache/poltter` (ili pokrenite `rm -rf ~/.cache/poltter`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\poltter`

3. Ponovo pokrenite Poltter Desktop.

---

### Rješavanje problema sa vezom na serveru

Poltter Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `poltter.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `POLTTER_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `POLTTER_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Poltter Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Poltter karakteristikama.

---

### Obavještenja se ne prikazuju

Poltter Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Poltter u postavkama vašeg OS-a, i
...
1. Zatvorite Poltter Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Poltter Desktop aplikacije):

- `poltter.settings.dat` (URL zadanog servera za desktop)
- `poltter.global.dat` i `poltter.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `poltter models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\poltter`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Poltter dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\poltter`

2. Ponovo pokrenite Poltter da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Poltter da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Poltter će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
