...
Možete dodati vanjske alate u Poltter koristeći _Model Context Protocol_, ili MCP. Poltter podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Poltter Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/poltter`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Poltter automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Poltter će:
...
Ako server zahtijeva autentifikaciju, Poltter će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `poltter mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Poltter sigurno čuva tokene u `~/.local/share/poltter/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Poltter, zajedno s ugrađenim alatima. Možete njima upravljati kroz Poltter konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Poltter sa vašim Sentry nalogom.
