---
title: Server
description: Komunicirajte s poltter serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `poltter serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji poltter klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `poltter.local` |
...
Postavite `POLTTER_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `poltter`, ili postavite `POLTTER_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `poltter serve` i za `poltter web`.
...
Kada pokrenete `poltter`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite poltter server za programsku interakciju sa poltter-om.
:::

Ova arhitektura omogucava poltter podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `poltter serve` da startate standalone server. Ako je poltter TUI vec pokrenut, `poltter serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Poltter [IDE](/docs/ide) pluginovi.
...
poltter server izlaže sljedece API-je.
