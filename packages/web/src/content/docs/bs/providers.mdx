---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Poltter.
---

import config from "../../../../config.mjs"
export const console = config.console

Poltter koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Poltter konfiguraciji.
...
u `~/.local/share/poltter/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Poltter
config.
...
Poltter Zen je lista modela koje je obezbedio Poltter tim koji su bili
testirano i potvrđeno da dobro radi sa Poltter. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Poltter Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite poltter i idite na [opencode.ai/auth](https://opencode.ai/auth).
...
Radi kao i svaki drugi provajder u Poltter i potpuno je opcionalan za korištenje.

---

## Poltter Go

Poltter Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Poltter tim i koji su testirani i verificirani da dobro rade s Poltter-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Poltter Go` i idite na [opencode.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s Poltter:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete poltter:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `poltter.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Poltter.
:::
...
Možete konfigurirati poltter za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi poltter ispravno radio.
   :::
...
Također možete dodati modele kroz svoju poltter konfiguraciju.
...
da pohrani token u poltter auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Poltter koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Poltter
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`poltter.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `poltter.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s poltter:
...
Za korištenje Google Vertex AI s Poltter:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće poltter.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem polttera, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [poltter-helicone-session](https://github.com/H2Shami/poltter-helicone-session) da automatski prijavite svaki Poltter razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Poltter razgovor naveden kao zasebna sesija.
...
Možete konfigurirati poltter za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati poltter za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati poltter za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Poltter. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/poltter) za detalje.
:::
...
Da biste koristili Ollama Cloud s Poltter:
...
3. Kopirajte API ključ za korištenje u Poltter.
...
6. **Važno**: Prije upotrebe modela oblaka u Poltter, morate lokalno povući informacije o modelu:
...
Poltter Zen je lista testiranih i verifikovanih modela koju je obezbedio Poltter tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Poltter Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Poltter Zen**.

```txt
   /connect
```

3. Unesite svoj Poltter API ključ.
...
Također možete dodati dodatne modele putem vaše poltter konfiguracije.
...
5. Također ih možete prilagoditi putem vaše poltter konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Poltterom:
...
Također možete prilagoditi modele kroz svoju poltter konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s poltter-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `poltter.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Poltter da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `poltter auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Poltter konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj poltter konfiguraciji.
