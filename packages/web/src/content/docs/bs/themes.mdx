...
U Poltter mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Poltter koristi nasu `poltter` temu.
...
Poltter dolazi sa vise ugradenih tema.
...
- Zele da Poltter odgovara izgledu njihovog terminala
...
Poltter podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/poltter/themes/*.json` ili `$XDG_CONFIG_HOME/poltter/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.poltter/themes/*.json`
4. **Trenutni radni direktorij** - `./.poltter/themes/*.json`
