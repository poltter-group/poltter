---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Poltter.
---

Dodaci vam omogućavaju da proširite Poltter spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Poltter.
...
- `.poltter/plugins/` - Dodaci na nivou projekta
- `~/.config/poltter/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/poltter/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/poltter/poltter.json`)
2. Konfiguracija projekta (`poltter.json`)
3. Globalni direktorij dodataka (`~/.config/poltter/plugins/`)
4. Direktorij dodataka projekta (`.poltter/plugins/`)
...
Poltter pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Poltter SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Poltter.
...
Spriječite poltter da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u poltter:
...
Pomoćnik `tool` kreira prilagođeni alat koji poltter može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za poltter zajedno sa ugrađenim alatima.
