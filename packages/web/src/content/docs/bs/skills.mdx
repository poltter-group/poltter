...
Agent skills omogucavaju Poltter da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Poltter pretrazuje ove lokacije:

- Konfiguracija projekta: `.poltter/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/poltter/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Poltter ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.poltter/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/poltter/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.poltter/skills/git-release/SKILL.md` ovako:
...
Poltter navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `poltter.json`:
...
**Za ugradene agente** (u `poltter.json`):
