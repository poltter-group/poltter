...
Postoje dvije vrste agenata u Poltter: primarni agenti i podagenti.
...
Poltter dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Poltter dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Poltter dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u Poltter-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `poltter.json`:
...
- Globalno: `~/.config/poltter/agents/`
- Po projektu: `.poltter/agents/`

```markdown title="~/.config/poltter/agents/review.md"
---
...
Ako temperatura nije navedena, Poltter koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Poltter konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Poltter konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Poltter Zen](/docs/zen), koristili biste `poltter/gpt-5.1-codex` za GPT 5.1 Codex.
...
```json title="poltter.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="poltter.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Pokrenite `poltter models` da vidite listu dostupnih modela.
:::
...
```markdown title="~/.config/poltter/agents/docs-writer.md"
---
...
```markdown title="~/.config/poltter/agents/security-auditor.md"
---
