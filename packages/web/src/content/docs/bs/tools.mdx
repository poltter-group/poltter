...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Poltter dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Poltter.
...
:::note
Ovaj alat je dostupan samo kada je `POLTTER_EXPERIMENTAL_LSP_TOOL=true` (ili `POLTTER_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Poltter provajdera ili kada je varijabla `POLTTER_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Poltter:
