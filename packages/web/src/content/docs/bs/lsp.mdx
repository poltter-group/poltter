---
title: LSP serveri
description: Poltter se integriše sa vašim LSP serverima.
---

Poltter se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

Poltter dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `POLTTER_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i poltter otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj poltter konfiguraciji.
