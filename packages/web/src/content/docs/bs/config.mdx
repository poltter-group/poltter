---
title: Konfiguracija
description: Korištenje Poltter JSON konfiguracije.
---

Možete konfigurirati Poltter koristeći JSON konfiguracijski fajl.

---

## Format

Poltter podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/poltter`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/poltter/poltter.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`POLTTER_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`poltter.json` u projektu) - postavke specifične za projekat
5. **`.poltter` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`POLTTER_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.poltter` i `~/.config/poltter` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/poltter` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Poltter konfiguraciju u `~/.config/poltter/poltter.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/poltter/tui.json`.
...
Dodajte `poltter.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Poltter pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `POLTTER_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `POLTTER_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.poltter` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.poltter`, tako da **može nadjačati** njihove postavke.
...
Koristite `POLTTER_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `poltter.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `poltter serve` i `poltter web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Poltter server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `poltter.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Poltter konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Poltter pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/poltter/agents/` ili `.poltter/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Poltter će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`poltter run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/poltter/commands/` ili `.poltter/commands/`. [Saznajte više ovdje](/docs/commands).
...
Poltter će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Poltter **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Poltter sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.poltter/plugins/` ili `~/.config/poltter/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Poltter da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
