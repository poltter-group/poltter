...
> current private `@poltter-ai/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@poltter-ai/ai`
...
- Secondary audience: Poltter and other durable agent runtimes
...
import { LLM } from "@poltter-ai/ai"
import { OpenAI } from "@poltter-ai/ai/providers/openai"
...
```ts
import { Message } from "@poltter-ai/ai"
...
import { LLM, Tool } from "@poltter-ai/ai"
...
Poltter and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@poltter-ai/ai/promise"
import { OpenAI } from "@poltter-ai/ai/providers/openai"
...
```ts
import { LLMSchema } from "@poltter-ai/ai/schema"
...
```ts
import { Provider, Protocol } from "@poltter-ai/ai/provider"
...
@poltter-ai/ai/promise
  Promise/AsyncIterable LLM facade

@poltter-ai/ai/schema
  serializable domain schemas

@poltter-ai/ai/provider
  experimental Provider and Protocol authoring APIs

@poltter-ai/ai/providers/openai
@poltter-ai/ai/providers/anthropic
@poltter-ai/ai/providers/google
...
```
...
Poltter should migrate to `generateTurn` / `streamTurn`, preserving its durable
