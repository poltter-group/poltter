...
This is the living design and status document for `@poltter-ai/codemode` and its existing V2 Poltter adapter.
...
`packages/poltter/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
