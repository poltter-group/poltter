...
export const POLTTER_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const POLTTER_MODE_KEY = "poltter.mode"

export const PoltterKeymapProvider = KeymapProvider
export const usePoltterKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type PoltterModeStack = ReturnType<typeof createPoltterModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, PoltterModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createPoltterModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(POLTTER_MODE_KEY, POLTTER_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(POLTTER_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(POLTTER_MODE_KEY, stack.at(-1)?.mode ?? POLTTER_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? POLTTER_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(POLTTER_MODE_KEY, undefined)
...
export function usePoltterModeStack() {
  return getPoltterModeStack(usePoltterKeymap())
}

export function getPoltterModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Poltter mode stack is not registered for this keymap")
  return value
}
...
export function registerPoltterKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createPoltterModeStack(keymap)
