import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@poltter-ai/plugin/tui"
...
import type { usePoltterKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof usePoltterKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getPoltterModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getPoltterModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
