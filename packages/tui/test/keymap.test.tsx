...
import { getPoltterModeStack, POLTTER_BASE_MODE, PoltterKeymapProvider, registerPoltterKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerPoltterKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when poltter mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getPoltterModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getPoltterModeStack(keymap).push("autocomplete")
