...
- Author package entrypoint is `@poltter-ai/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/poltter-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@poltter-ai/plugin/tui`.
- `@poltter-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@poltter-ai/plugin/tui"
...
- The value is a semver range checked against the running Poltter version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.poltter` is absent, no check is performed (backward compatible).
...
- `poltter plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `poltter plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.poltter` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.poltter`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.poltter`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.poltter` dir or `POLTTER_CONFIG_DIR`, Poltter installs `@poltter-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@poltter-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Poltter's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Poltter registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Poltter pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@poltter-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"poltter"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `poltter.default`.
...
- Local plugins persist installed themes under the local `.poltter/themes` area near the plugin config source.
...
- `--pure` / `POLTTER_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.poltter/plugins/tui-smoke.tsx`
- Local vim plugin: `.poltter/plugins/tui-vim.tsx`
- Local smoke config: `.poltter/tui.json`
- Local smoke theme: `.poltter/plugins/smoke-theme.json`
