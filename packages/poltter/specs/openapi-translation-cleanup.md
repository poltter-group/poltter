...
Trim `packages/poltter/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/poltter/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/poltter`.
- `[x]` `bun typecheck` from `packages/poltter`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/poltter/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/poltter`.
- `[x]` `bun dev generate > /tmp/poltter-openapi.json` from `packages/poltter`.
...
- Edit `packages/poltter/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/poltter-openapi.json` from `packages/poltter`.
...
- Check source schemas in files like `packages/poltter/src/session/schema.ts`, `packages/poltter/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/poltter/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/poltter/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
