...
Practical reference for a future `packages/server` split after the poltter
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/poltter`.
...
Until enough shared service code lives outside `packages/poltter`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/poltter`

It should not import `packages/poltter` services while `packages/poltter`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on poltter-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/poltter` implementation details.
