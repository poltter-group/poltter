...
Delete `packages/poltter/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/poltter/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/poltter/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/poltter`
- TUI plugin loader tests in `packages/poltter` if runtime plugin API wiring changed
