...
describe("poltter acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ poltter }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ poltter }))
        ...
        expect(initialized.agentInfo?.name).toBe("Poltter")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ poltter }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ poltter })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("poltter-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "poltter-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.POLTTER_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ poltter }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("poltter-login")
