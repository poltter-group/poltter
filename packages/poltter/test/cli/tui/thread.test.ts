...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ poltter }) =>
    Effect.gen(function* () {
      const result = yield* poltter.spawn(["--replay-limit", "10"])

      poltter.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ poltter }) =>
    Effect.gen(function* () {
      const result = yield* poltter.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ poltter }) =>
    Effect.gen(function* () {
      const result = yield* poltter.spawn(["--mini", "--port", "4096"])
