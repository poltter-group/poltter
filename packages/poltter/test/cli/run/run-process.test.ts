// Subprocess integration tests for `poltter run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `poltter.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `POLTTER_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("poltter run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, poltter }) =>
      Effect.gen(function* () {
        ...
        const result = yield* poltter.run("say hi")
        poltter.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, poltter }) =>
      Effect.gen(function* () {
        ...
        const result = yield* poltter.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, poltter }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* poltter.run("think", { extraArgs: ["--thinking"] })
        poltter.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* poltter.run("think again")
        poltter.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, poltter }) =>
      Effect.gen(function* () {
        ...
        const result = yield* poltter.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, poltter }) =>
      Effect.gen(function* () {
        ...
        const result = yield* poltter.run("say hi", { format: "json" })
        poltter.expectExit(result, 0)

        const events = poltter.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ poltter }) =>
      Effect.gen(function* () {
        const result = yield* poltter.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, poltter }) =>
      Effect.gen(function* () {
        ...
        const result = yield* poltter.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, poltter }) =>
      Effect.gen(function* () {
        ...
        const result = yield* poltter.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, poltter }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* poltter.run("request permission", { permission: { bash: "ask" } })
        poltter.expectExit(denied, 0)
        ...
        const allowed = yield* poltter.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        poltter.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* poltter.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        poltter.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, poltter }) =>
      Effect.gen(function* () {
        ...
        const server = yield* poltter.serve()

        const result = yield* poltter.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, poltter }) =>
      Effect.gen(function* () {
        const result = yield* poltter.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, poltter }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* poltter.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
