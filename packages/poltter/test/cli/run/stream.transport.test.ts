...
import { PoltterClient, type GlobalEvent } from "@poltter-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<PoltterClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<PoltterClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<PoltterClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<PoltterClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<PoltterClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://poltter.test"),
...
function sdk(
  input: {
    ...
    subscribe?: PoltterClient["event"]["subscribe"]
    globalEvent?: PoltterClient["global"]["event"]
    promptAsync?: PoltterClient["session"]["promptAsync"]
    status?: PoltterClient["session"]["status"]
    messages?: PoltterClient["session"]["messages"]
    children?: PoltterClient["session"]["children"]
    permissions?: PoltterClient["permission"]["list"]
    questions?: PoltterClient["question"]["list"]
  } = {},
) {
  const client = new PoltterClient()

  const subscribe: PoltterClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: PoltterClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: PoltterClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: PoltterClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: PoltterClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: PoltterClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: PoltterClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: PoltterClient["question"]["list"] = input.questions ?? (() => ok([]))
