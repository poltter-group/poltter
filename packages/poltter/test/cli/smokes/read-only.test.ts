...
describe("poltter read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // POLTTER_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ poltter }) =>
      Effect.gen(function* () {
        const r = yield* poltter.spawn(["mcp", "list"])
        poltter.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ poltter }) =>
      Effect.gen(function* () {
        const r = yield* poltter.spawn(["providers", "list"])
        poltter.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ poltter }) =>
      Effect.gen(function* () {
        const r = yield* poltter.spawn(["models"])
        poltter.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ poltter }) =>
      Effect.gen(function* () {
        const r = yield* poltter.spawn(["agent", "list"])
        poltter.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh POLTTER_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ poltter }) =>
      Effect.gen(function* () {
        const r = yield* poltter.spawn(["session", "list"])
        poltter.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ poltter }) =>
      Effect.gen(function* () {
        const r = yield* poltter.spawn(["stats"])
        poltter.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under POLTTER_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ poltter }) =>
      Effect.gen(function* () {
        const r = yield* poltter.spawn(["db", "path"])
        poltter.expectExit(r, 0, "db path")
