...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/poltter.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/poltter.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/poltter.jsonc", dir: ".poltter", suggestion: "poltter" },
        expected:
          'Directory ".poltter" in /tmp/poltter.jsonc is not valid. Rename the directory to "poltter" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/poltter.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/poltter.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `poltter models` to list available models",
      "Or check your config (poltter.json) provider/model names",
    ].join("\n")
