...
import type { Event, PoltterClient } from "@poltter-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<PoltterClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "poltter-acp-permission-"))
