...
it.instance("plan agent denies edits except .poltter/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".poltter/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".poltter", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.POLTTER_TEST_HOME
      process.env.POLTTER_TEST_HOME = test.directory
