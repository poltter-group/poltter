...
describe("HttpApi workspace proxy", () => {
  ...
  it.live("strips poltter-internal headers and merges extra headers", () =>
    Effect.gen(function* () {
      ...
      expect(forwarded["x-poltter-directory"]).toBeUndefined()
      expect(forwarded["x-poltter-workspace"]).toBeUndefined()
