...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "poltter.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "poltter.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "poltter.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "poltter.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["poltter.json", "poltter.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "poltter.json"),
        path.join(project, "poltter.jsonc"),
      ])
    })
  })
