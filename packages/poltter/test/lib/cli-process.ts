// Subprocess test harness for the poltter CLI. Spawns the real binary against
...
//
// Configuration flows through poltter's built-in test affordances:
//   - POLTTER_CONFIG_CONTENT      : provider config inline, no files to find
//   - POLTTER_TEST_HOME           : pins os.homedir() → tmpdir
//   - POLTTER_DISABLE_PROJECT_CONFIG : skip walking up for poltter.json
//   - POLTTER_PURE                : skip external plugin discovery + install
//   - POLTTER_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `poltter.run` is fully wired. The shape supports adding more
// builders (`poltter.serve(opts)`, `poltter.acp(opts)`, `poltter.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of PoltterCli.
...
import { AppProcess } from "@poltter-ai/core/process"
...
const poltterRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(poltterRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    POLTTER_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    POLTTER_CONFIG_CONTENT: configJson,
    POLTTER_DISABLE_PROJECT_CONFIG: "1",
    POLTTER_PURE: "1",
    POLTTER_DISABLE_AUTOUPDATE: "1",
    POLTTER_DISABLE_AUTOCOMPACT: "1",
    POLTTER_DISABLE_MODELS_FETCH: "1",
    POLTTER_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `poltter run`. New flags should
...
// `poltter serve` is a long-lived process — it never exits on its own.
...
// `poltter acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type PoltterCli = {
  ...
  // Spawn `poltter serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `poltter acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("poltter.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          POLTTER_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("poltter.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("poltter.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "poltter server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `poltter serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("poltter.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const poltter: PoltterCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, poltter })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `poltter.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "poltter") {
...
// every call site. Use this for any test that needs the poltter CLI fixture.
//
...
// resources (e.g. `poltter.serve`) without an extra `Effect.scoped` wrapper —
