...
import { isRecord } from "@poltter-ai/tui/util/record"
...
import { TuiKeybind } from "@poltter-ai/tui/config/keybind"
...
import { TuiConfig } from "@poltter-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@poltter/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old poltter.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.poltter`
  // folders between cwd and home, and POLTTER_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.POLTTER_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit POLTTER_TUI_CONFIG override, if set.
  if (Flag.POLTTER_TUI_CONFIG) {
    const configFile = Flag.POLTTER_TUI_CONFIG
  ...
  // 4. `.poltter` directories (and POLTTER_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".poltter") || dir === Flag.POLTTER_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".poltter") && dir !== Flag.POLTTER_CONFIG_DIR) continue
