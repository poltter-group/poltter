...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from poltter.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const poltter = await poltterFiles(input)
  for (const file of poltter) {
...
async function poltterFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "poltter"),
    ...(await Filesystem.findUp(["poltter.json", "poltter.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "poltter"))
  }
  if (Flag.POLTTER_CONFIG) files.push(Flag.POLTTER_CONFIG)
