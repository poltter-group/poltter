...
import { FrontmatterError } from "@poltter-ai/core/v1/config/error"
import { ConfigMarkdown } from "@/config/markdown"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Glob } from "@poltter-ai/core/util/glob"
...
const POLTTER_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with poltter. The model's intuition for what an
// poltter.json should look like is often wrong, and poltter hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch poltter's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_POLTTER_SKILL_NAME = "customize-poltter"
const CUSTOMIZE_POLTTER_SKILL_DESCRIPTION =
  "Use ONLY when the user is editing or creating poltter's own configuration: poltter.json, poltter.jsonc, files under .poltter/, or files under ~/.config/poltter/. Also use when creating or fixing poltter agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring poltter itself."
const CUSTOMIZE_POLTTER_SKILL_BODY = SkillPlugin.CustomizePoltterContent
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, POLTTER_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@poltter/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_POLTTER_SKILL_NAME] = {
          name: CUSTOMIZE_POLTTER_SKILL_NAME,
          description: CUSTOMIZE_POLTTER_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_POLTTER_SKILL_BODY,
        }
