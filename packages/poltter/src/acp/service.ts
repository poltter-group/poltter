...
import type { AssistantMessage, Message, PoltterClient, SessionMessageResponse } from "@poltter-ai/sdk/v2"
...
export const AuthMethodID = "poltter-login"
...
export class Service extends Context.Service<Service, Interface>()("@poltter/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `poltter auth login` in the terminal",
      name: "Login with poltter",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: PoltterClient) {
...
function makeUsageService(sdk: PoltterClient) {
...
async function loadDirectorySnapshot(sdk: PoltterClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, poltter provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const poltterProvider = providers[ProviderV2.ID.make("poltter")]
  const poltterModel = poltterProvider ? Provider.sort(Object.values(poltterProvider.models))[0] : undefined
  if (poltterProvider && poltterModel) return { providerID: poltterProvider.id, modelID: poltterModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Poltter service failure", service })
}
