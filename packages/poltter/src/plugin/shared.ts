...
export const DEPRECATED_PLUGIN_PACKAGES = ["poltter-openai-codex-auth", "poltter-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, poltterVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(poltterVersion) || semver.major(poltterVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.poltter
  if (typeof range !== "string") return
  if (!semver.satisfies(poltterVersion, range)) {
    throw new Error(`Plugin requires poltter ${range} but running ${poltterVersion}`)
  }
}
