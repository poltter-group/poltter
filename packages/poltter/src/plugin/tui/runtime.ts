...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@poltter-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@poltter-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@poltter-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@poltter-ai/tui/context/theme"
import { Global } from "@poltter-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@poltter-ai/core/util/flock"
import { Flag } from "@poltter-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@poltter-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@poltter-ai/core/v1/config/plugin"
import { createCommandShim } from "@poltter-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@poltter-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".poltter"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".poltter", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".poltter", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.POLTTER_PURE ? [] : pluginOrigins
    if (Flag.POLTTER_PURE && pluginOrigins.length) {
    }
