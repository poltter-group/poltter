...
import { PoltterHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for poltter serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(PoltterHttpApi)
...
export const PublicApi = PoltterHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "poltter",
    version: "1.0.0",
    description: "poltter api",
