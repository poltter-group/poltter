import type { PermissionV1 } from "@poltter-ai/core/v1/permission"
import { FSUtil } from "@poltter-ai/core/fs-util"
// CLI entry point for `poltter run` and `poltter --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`poltter --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`poltter --mini --attach`): connects to a running
//      poltter server and runs interactive mode against it.
//
...
import { createPoltterClient, type PoltterClient, type ToolPart } from "@poltter-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<PoltterClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run poltter with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running poltter server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to POLTTER_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to POLTTER_SERVER_USERNAME or 'poltter')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: PoltterClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: PoltterClient, sessionID: string) {
      ...
      async function current(sdk: PoltterClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: PoltterClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: PoltterClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: PoltterClient) {
        ...
        async function loop(client: PoltterClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createPoltterClient({
        baseUrl: "http://poltter.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
