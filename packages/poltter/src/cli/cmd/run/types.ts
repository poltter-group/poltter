// Shared type vocabulary for the direct interactive mode (`poltter --mini`).
//
...
import type { PoltterClient, PermissionRequest, QuestionRequest, ToolPart } from "@poltter-ai/sdk/v2"
import type { TuiConfig } from "@poltter-ai/tui/config"
...
type PromptModel = Parameters<PoltterClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<PoltterClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<PoltterClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<PoltterClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<PoltterClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<PoltterClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<PoltterClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<PoltterClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<PoltterClient["question"]["reject"]>[0]
