...
//
// Renders the full poltter entry logo and a compact [O] exit badge, plus
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  const top = 1
  const leftWidth = Math.max(...go.left.map((row) => row.length)) + 1
  const body_left = leftWidth + Math.max(...go.right.map((row) => row.length)) + 2
  let height = 1

  go.left.forEach((row, index) => {
    draw(lines, row, {
      left: 0,
      top: top + index,
      fg: theme.left,
      shadow: theme.leftShadow,
    })
  })

  go.right.forEach((row, index) => {
    draw(lines, row, {
      left: leftWidth,
      top: top + index,
      fg: theme.right,
      shadow: theme.rightShadow,
    })
  })
  ...
  if (kind === "exit") {
    ...
    if (input.showSession !== false) {
      push(lines, body_left, top + 1, session, theme.left, undefined, TextAttributes.DIM)
      push(lines, body_left + session.length, top + 1, meta.title, theme.right, undefined, TextAttributes.BOLD)
    }

    push(lines, body_left, top + 2, label, theme.left, undefined, TextAttributes.DIM)
    push(
      lines,
      body_left + label.length,
      top + 2,
      `poltter --mini -s ${meta.session_id}`,
      theme.right,
      undefined,
      TextAttributes.BOLD,
    )
    height = top + go.left.length + 1
  }
