...
import type { LLMClientShape } from "@poltter-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("poltter"))
    return { type: "unsupported", reason: "provider is not openai, poltter, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @poltter-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains poltter-owned. The native runtime only adapts
      // the @poltter-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
