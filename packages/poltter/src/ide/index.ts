...
import { IdeEvent } from "@poltter-ai/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["POLTTER_CALLER"] === "vscode" || process.env["POLTTER_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.poltter"], {
    nothrow: true,
  })
