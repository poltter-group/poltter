...
import type { UpdaterState } from "@poltter-ai/app/updater"
...
import { useTheme } from "@poltter-ai/ui/theme/context"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.POLTTER_CHANNEL === "prod" &&
...
const deepLinkEvent = "poltter:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__POLTTER__ ??= {}
  const pending = window.__POLTTER__.deepLinks ?? []
  window.__POLTTER__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `poltter.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, onClick) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://poltter.ai/favicon-96x96-v3.png",
      })
...
function LoadingSplash() {
  return (
    <div class="h-dvh w-screen flex flex-col items-center justify-center bg-background-base">
      <img src="./poltter-logo.png" alt="Poltter" class="w-20 h-20 opacity-60 animate-pulse" />
    </div>
  )
}
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("poltter.global.dat").getItem("language")
  ...
  function App() {
    ...
    const [splashSettled, setSplashSettled] = createSignal(false)
    let didSchedule = false
    createEffect(() => {
      const allLoaded = !defaultServer.loading && !sidecar.loading && !locale.loading && !wslServers.isLoading
      if (!didSchedule && allLoaded) {
        didSchedule = true
        setTimeout(() => setSplashSettled(true), 2000)
      }
    })
    const ready = createMemo(
      () => !defaultServer.loading && !sidecar.loading && !locale.loading && !wslServers.isLoading && (splashSettled() || didSchedule),
    )
