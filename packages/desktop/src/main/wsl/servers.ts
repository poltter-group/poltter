...
import { expectPoltterVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolvePoltter?: typeof resolveWslPoltter
...
export function createWslServersController(
  ...
  const setPoltterCheck = (distro: string, check: WslPoltterCheck) => {
    setState({
      poltterChecks: {
        ...state.poltterChecks,
        [distro]: check,
  ...
  const checkPoltter = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolvePoltter ?? resolveWslPoltter)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return poltterCheck(distro, resolved, version, appVersion)
  }

  const refreshPoltterCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setPoltterCheck(distro, await checkPoltter(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const poltterChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.poltterChecks[distro])
        .map(async (distro) => [distro, await checkPoltter(distro, opts)] as const),
    )
    if (poltterChecks.length) {
      setState({ poltterChecks: { ...state.poltterChecks, ...Object.fromEntries(poltterChecks) } })
    }
  }
  ...
  const refreshPoltterCheckBackground = (id: string, distro: string) => {
    void checkPoltter(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setPoltterCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl poltter check failed", { id, distro, message })
      })
  }

  const refreshPoltterChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkPoltter(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setPoltterCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl poltter check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshPoltterCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installPoltter(name: string) {
      await runJob({ kind: "install-poltter", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslPoltter(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "Poltter installation failed")
        }
        await refreshPoltterCheck(name, { signal: abort.signal })
        expectPoltterVersion(state.poltterChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.poltterChecks, distro) : {}),
      })
    },
...
function poltterCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "poltter is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "poltter is installed but could not run",
    }
  }
  return {
    distro,
