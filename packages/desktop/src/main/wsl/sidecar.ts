...
import { type WslCommandLine, resolveWslPoltter, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const poltter = await resolveWslPoltter(distro)
  if (!poltter) throw new Error(`Poltter is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "poltter"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export POLTTER_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export POLTTER_CLIENT=desktop",
    `export POLTTER_SERVER_USERNAME=${shellEscape(username)}`,
    `export POLTTER_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(poltter)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
