...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.poltter.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `poltter.settings.dat` is special: it maps to the `poltter.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"poltter.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // poltter.settings.dat → the electron settings store ("poltter.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "poltter.global.dat").
  const storeName = filename === "poltter.settings.dat" ? "poltter.settings" : filename
