...
import type { DesktopMenuAction } from "@poltter-ai/app/desktop-menu"
...
const ALLOWED_STORE_PREFIXES = ["poltter.window.", "poltter.workspace.", "poltter.draft."]

function requireAllowedStore(name: string) {
  if (ALLOWED_STORE_NAMES.has(name)) return
  if (ALLOWED_STORE_PREFIXES.some((prefix) => name.startsWith(prefix) && name.endsWith(".dat"))) return
  throw new Error(`Store access denied: ${name}`)
}
...
export function registerIpcHandlers(deps: Deps) {
  ...
  ipcMain.handle("open-local-file", (_event: IpcMainInvokeEvent, url: string) => {
    openLocalFileURL(url)
  })

  ipcMain.handle("open-path", async (_event: IpcMainInvokeEvent, path: string, app?: string) => {
    if (!app) return shell.openPath(path)
    // Only allow known-safe apps to prevent arbitrary command execution.
    const safeApps = new Set(["code", "code-insiders", "vim", "nvim", "nano", "notepad", "notepad++", "sublime_text", "atom"])
    if (!safeApps.has(app)) {
      writeLog("ipc", "open-path blocked unknown app", { app, path }, "warn")
      return
    }
