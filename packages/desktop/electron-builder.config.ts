...
const getBase = (appId: string): Configuration => ({
  artifactName: "poltter-desktop-${os}-${arch}.${ext}",
  directories: {
    output: "dist",
  ...
  files: ["out/**/*", "resources/**/*", "!resources/poltter-cli*"],
  extraResources: [
    ...(channel === "dev"
      ? [
          {
  ...
  switch (channel) {
    case "dev": {
      return {
        ...base,
        appId,
        productName: "Poltter Dev",
        deb: { fpm: [metainfoFpm(appId)] },
        rpm: { packageName: "poltter-dev", fpm: [metainfoFpm(appId)] },
      }
    }
    case "beta": {
      return {
