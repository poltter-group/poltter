...
import { createPoltter, type ToolPart } from "@poltter-ai/sdk"
...
console.log("🚀 Starting poltter server...")
const poltter = await createPoltter({
  port: 0,
})
console.log("✅ Poltter server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await poltter.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new poltter session...")
    const { client, server } = poltter
    ...
    console.log("✅ Created poltter session:", createResult.data.id)
  ...
  console.log("📝 Sending to poltter:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Poltter response:", JSON.stringify(result, null, 2))
