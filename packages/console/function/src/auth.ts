...
import { Account } from "@poltter-ai/console-core/account.js"
...
import { User } from "@poltter-ai/console-core/user.js"
import { and, Database, eq, isNull, or } from "@poltter-ai/console-core/drizzle/index.js"
...
import { AuthTable } from "@poltter-ai/console-core/schema/auth.sql.js"
...
const MY_THEME: Theme = {
  ...THEME_OPENAUTH,
  logo: "https://poltter.ai/favicon-v3.svg",
}
      ...
      providers: {
        ...
        email: CodeProvider({
          async request(req, state, form, error) {
            ...
            if (state.type === "start") {
              return Response.redirect(Resource.App.url + "/auth/email?" + params.toString(), 302)
            }

            if (state.type === "code") {
              return Response.redirect(Resource.App.url + "/auth/code?" + params.toString(), 302)
            }
          ...
          async sendCode(claims, code) {
            ...
            const cmd = new SendEmailCommand({
              ...
              FromEmailAddress: `Poltter <auth@${Resource.App.domain}>`,
              Content: {
                Simple: {
                  Body: {
                    Html: {
