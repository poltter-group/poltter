...
import { and, Database, eq, isNull, lt, or, sql } from "@poltter-ai/console-core/drizzle/index.js"
import { KeyTable } from "@poltter-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@poltter-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@poltter-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@poltter-ai/console-core/util/date.js"
...
import { ZenData } from "@poltter-ai/console-core/model.js"
import { Subscription } from "@poltter-ai/console-core/subscription.js"
import { BlackData } from "@poltter-ai/console-core/black.js"
import { UserTable } from "@poltter-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@poltter-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@poltter-ai/console-core/schema/provider.sql.js"
...
import { Workspace } from "@poltter-ai/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-poltter-session") ?? ""
    const requestId = input.request.headers.get("x-poltter-request") ?? ""
    const ocClient = input.request.headers.get("x-poltter-client") ?? ""
    const projectId = input.request.headers.get("x-poltter-project") ?? ""
    ...
    if (
      ...
      throw new RegionError(
        t("zen.api.error.regionNotAllowed", {
          consoleGoUrl: `https://poltter.ai/workspace/${authInfo.workspaceID}/go`,
        }),
      )
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWithRetryableStatus(
        reqUrl,
        {
          ...
          headers: (() => {
            ...
            headers.delete("x-poltter-request")
            headers.delete("x-poltter-session")
            headers.delete("x-poltter-project")
            headers.delete("x-poltter-client")
            return headers
          })(),
          body: reqBody,
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-poltter-endpoint-id")
        const resEndpointModelId = res.headers.get("x-poltter-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://poltter.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://poltter.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://poltter.ai/workspace/${authInfo.workspaceID}/members`
