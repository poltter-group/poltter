...
import { createPoltterClient } from "@poltter-ai/sdk/v2/client"
import { ServerAuth } from "@poltter-ai/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createPoltterClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@poltter/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createPoltterClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createPoltterClient({ baseUrl: connection.url, headers: connection.headers })
    })
