...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("poltter-find")
  api.delete("poltter-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("poltter-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("poltter-find", new Highlight(...rest))
    return true
  }
