...
describe("inlineCodeKind", () => {
  test("leaves code expressions as normal inline code", () => {
    ...
    expect(inlineCodeKind(`@poltter-ai/app <StatusPopover />)`)).toBeUndefined()
  ...
  test("detects file and directory paths", () => {
    ...
    expect(inlineCodeKind(`~/.config/poltter`)).toBe("path")
    expect(inlineCodeKind(`@poltter-ai/app`)).toBe("path")
  ...
  test("detects urls", () => {
    ...
    expect(inlineCodeKind(`file:///tmp/poltter`)).toBeUndefined()
