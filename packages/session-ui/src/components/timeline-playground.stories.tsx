...
function normalize(raw: unknown) {
  ...
  if (!record(raw) || !record(raw.info) || typeof raw.info.id !== "string" || !Array.isArray(raw.messages)) {
    throw new Error("Expected an `poltter export` JSON file")
  }
...
function Playground() {
  ...
  const data = createMemo(() => ({
    session: [session()],
    agent: [{ name: "build" }, { name: "plan" }, { name: "chat" }, { name: "agent" }, { name: "searcher" }, { name: "review" }, { name: "explore" }],
    session_status: {},
    session_diff: {},
          ...
          <Show when={panels.generators}>
            <div style={{ padding: "0 12px 12px", display: "flex", "flex-direction": "column", gap: "6px" }}>
              ...
              <div style={{ "font-size": "10px", color: "var(--text-weaker)", "margin-bottom": "2px" }}>
                Replaces the current timeline with an `poltter export` JSON file
              </div>
