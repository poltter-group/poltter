...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve poltter route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "poltter")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "poltter")).toBe("openai")
    expect(statProvider("big-pickle", "", "poltter")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "poltter"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep poltter as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "poltter"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "poltter"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep poltter or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "poltter"), country: "US" })).toMatchObject([
