...
stdenv.mkDerivation (finalAttrs: {
  ...
  desktopItems = lib.optional stdenv.hostPlatform.isLinux (makeDesktopItem {
    ...
    desktopName = "Poltter";
    exec = "poltter-desktop %U";
    icon = "ai.opencode.desktop";
    # Electron 41 derives X11 WM_CLASS from app.name.
    startupWMClass = "Poltter";
    ...
    + lib.optionalString stdenv.isLinux ''
      ...
      for file in "''${FILES[@]}"; do
        substituteInPlace $BASE_PATH/$file \
          --replace-fail "process.resourcesPath" "'$out/opt/poltter-desktop/resources'"
      done
    '';
  ...
  + lib.optionalString stdenv.hostPlatform.isDarwin ''
    ...
    makeWrapper "$out/Applications/Poltter.app/Contents/MacOS/Poltter" $out/bin/poltter-desktop
  ''
  + lib.optionalString stdenv.hostPlatform.isLinux ''
    mkdir -p $out/opt/poltter-desktop
    cp -r dist/linux*-unpacked/{resources,LICENSE*} $out/opt/poltter-desktop
    ...
    makeWrapper ${lib.getExe electron} $out/bin/poltter-desktop \
     --inherit-argv0 \
     --set ELECTRON_FORCE_IS_PACKAGED 1 \
     --add-flags $out/opt/poltter-desktop/resources/app.asar \
  ...
  meta = {
    description = "Poltter Desktop App";
    mainProgram = "poltter-desktop";
    inherit (poltter.meta) homepage license platforms;
  };
})
