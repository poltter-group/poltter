...
import { createPoltter } from "@poltter-ai/sdk"
...
`)
  ...
  const poltter = await createPoltter({ port: 0 })
  ...
  try {
    ...
    const session = await poltter.client.session.create()
    const result = await poltter.client.session
      .prompt({
