...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "poltter/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".poltter/glossary/zh-cn.md"
  if (locale === "zht") return ".poltter/glossary/zh-tw.md"
  return `.poltter/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Poltter did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Poltter returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Poltter session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Poltter runs for 'all' (default: 4)
      --model <provider/id>  Poltter model (default: poltter/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Poltter
...
`)
  ...
  if (failed.length) console.error(`\nPoltter failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.POLTTER_DISABLE_PROJECT_CONFIG = "1"
  env.POLTTER_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["poltter", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.POLTTER_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["poltter", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.POLTTER_CONFIG
  delete env.POLTTER_CONFIG_DIR
  delete env.POLTTER_CONFIG_CONTENT
  delete env.POLTTER_PERMISSION
  delete env.POLTTER_AUTO_SHARE
  return env
}
